...
const TextBlockRenderer: React.FC<{ block: TextBlock }> = ({ block }) => (
  <div className="prose max-w-none">
    ...
    <pre className="whitespace-pre-wrap font-sans text-gray-800 bg-gray-50 p-4 rounded-lg border border-gray-100">
      {block.markdown}
    </pre>
  </div>
);
...
const FindingBlockRenderer: React.FC<{ block: FindingBlock }> = ({ block }) => {
  ...
  const badgeClass = severityStyles[block.severity] || 'bg-gray-100 text-gray-800 border-gray-200';

  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-900 text-lg">{block.owaspCategory}</h3>
        <span className={`px-2.5 py-0.5 text-xs font-bold uppercase rounded-full border ${badgeClass}`}>
          {block.severity}
        </span>
      </div>
      
      <div className="text-sm font-mono text-gray-600 bg-gray-50 p-2.5 rounded border border-gray-100">
        <strong>File:</strong> {block.filePath} <span className="text-gray-400">|</span> <strong>Righe:</strong> {block.startLine} - {block.endLine}
      </div>
      
      <p className="text-gray-800 leading-relaxed">{block.explanation}</p>
...
const PolicyViolationBlockRenderer: React.FC<{ block: PolicyViolationBlock }> = ({ block }) => (
  <div className="flex flex-col gap-2">
    ...
    <p className="text-gray-800 mt-1"><strong>Regola violata:</strong> {block.ruleText}</p>
    <p className="text-gray-800">{block.explanation}</p>
    
    <div className="mt-3 text-sm text-gray-800 bg-yellow-50 p-3 border border-yellow-200 rounded shadow-sm">
...
const ChangelogItemBlockRenderer: React.FC<{ block: ChangelogItemBlock }> = ({ block }) => (
  <div className="flex flex-col gap-1.5 pb-3 border-b border-gray-100 last:border-0 last:pb-0">
    ...
    <p className="text-sm text-gray-600 pl-2.5 border-l-2 border-gray-200 ml-5">
      {block.detail}
    </p>
  </div>
);
