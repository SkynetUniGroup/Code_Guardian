...
import { SUPPORTED_PROVIDERS } from "../../credentials/supported-providers";
import type {
  RefEntry,
  RefSummary,
  RepositorySummary,
...
export class ServiceCredentialResponse implements ServiceCredentialDto {
  ...
  @ApiProperty({
    ...
    example: "GITHUB",
    description:
      "GITHUB (obbligatorio per le operazioni) o SONARQUBE (opzionale, arricchisce i prompt DOCS con le metriche di qualita').",
  })
  provider!: string;
