...
describe("ContextsService", () => {
  ...
  let model: {
    create: Mock<Promise<unknown>, [Record<string, unknown>]>;
  };
...
describe("TU_20 (RF.19,20,21,22,29,30) — arresto al primo passo fallito", () => {
  ...
  let credenziali: { getDecryptedToken: Mock };
  ...
  function tracciato(): string[] {
    ...
    return spie.filter(([, spia]) => spia.mock.calls.length > 0).map(([nome]) => nome);
  }

  beforeEach(async () => {
    credenziali = { getDecryptedToken: vi.fn().mockResolvedValue("token") };
    github = {
    ...
    const modulo: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: GithubClientService, useValue: github },
        { provide: FRANC, useValue: vi.fn().mockReturnValue("eng") },
      ],
    }).compile();
  ...
  describe("passo 1 — sintassi dell’URL (RF.19)", () => {
    ...
    it.each([
      ...
      await expect(pipe.transform({ ...dtoBase, repoUrl }, metadati)).rejects.toBeDefined();
      expect(tracciato()).toEqual([]);
    });

    it("lascia passare l’URL nella forma ammessa", async () => {
      await expect(pipe.transform({ ...dtoBase }, metadati)).resolves.toEqual(dtoBase);
    });
  });
  ...
  describe("passo 6 — ancoraggio dello SHA (RF.17)", () => {
    it("è derivato: nessuna chiamata in più, e l’albero viene chiesto sullo SHA ancorato", async () => {
      ...
      expect(github.getTree).toHaveBeenCalledWith("token", "owner", "repo", "sha-fissato");
    });

    it("senza commitSha si ancora alla testa del branch", async () => {
      await servizio.create("user1", dtoBase);

      expect(github.getTree).toHaveBeenCalledWith("token", "owner", "repo", SHA_HEAD);
    });
  });
  ...
  describe("passo 7 — rilevamento dei linguaggi (RF.24)", () => {
    it("un albero illeggibile ferma la sequenza prima della persistenza", async () => {
      ...
      await expect(servizio.create("user1", dtoBase)).rejects.toThrow("albero non leggibile");
  ...
  describe("passo 9 — esistenza dell’ambito (RF.30)", () => {
    it.each([
      ["un percorso assente dall’albero", { scopeType: "FILES" as const, paths: ["assente.ts"] }],
      ["un FILES che punta a una cartella", { scopeType: "FILES" as const, paths: ["src"] }],
      [
  ...
  describe("passo 10 — persistenza (RF.15)", () => {
    ...
    it("un fallimento della persistenza risale al chiamante invece di restituire un contesto finto", async () => {
      ...
      await expect(servizio.create("user1", dtoBase)).rejects.toThrow("mongo non raggiungibile");
    });
  });
});
