...
describe('RepositoriesService', () => {
  ...
  describe('list', () => {
    it("returns the caller's repositories using their decrypted token", async () => {
      ...
      expect(credentials.getDecryptedToken).toHaveBeenCalledWith(
        'user1',
        'GITHUB',
      );
  ...
  describe('refs', () => {
    it('resolves the repoUrl before listing refs', async () => {
      ...
      const result = await service.refs(
        'user1',
        'https://github.com/owner/repo',
      );

      expect(repoResolver.resolve).toHaveBeenCalledWith(
        'token',
        'https://github.com/owner/repo',
      );
  ...
  describe('tree', () => {
    ...
    it("resolves the branch's HEAD when no commitSha is given", async () => {
      ...
      expect(github.resolveRefToSha).toHaveBeenCalledWith(
        'token',
        'owner',
        'repo',
        'main',
