...
import { AgentRegistry, MAX_OPERATION_TIMEOUT_S } from "./agent-registry.service";
...
describe("AgentRegistry", () => {
  ...
  describe("the 300s hard ceiling (RQ.6, BE-15)", () => {
    ...
    it("keeps every shipped entry under the ceiling", () => {
      ...
      for (const code of codes) {
        expect(registry.getTimeoutS(code)).toBeLessThanOrEqual(MAX_OPERATION_TIMEOUT_S);
      }
    });
  });

  it("throws for an unknown operation code", () => {
    expect(() => registry.getTimeoutS("NOT_REAL" as never)).toThrow("Unknown OperationCode");
  });
  ...
  it("throws for an unknown operation code when looking up the owning agent", () => {
    expect(() => registry.getAgent("NOT_REAL" as never)).toThrow("Unknown OperationCode");
  });

  it("returns the human-readable display name for a known operation", () => {
    expect(registry.getDisplayName("DOCS_README")).toBe("README generation/update");
  });

  it("throws for an unknown operation code when looking up the display name", () => {
    expect(() => registry.getDisplayName("NOT_REAL" as never)).toThrow("Unknown OperationCode");
  });
});
...
describe("TU_18 (RF.40, RV.1) — descrittore risolto per tutti e sette gli OperationCode", () => {
  ...
  it.each(OPERATION_CODES)("%s risolve sull'agente e sul budget previsti", (code) => {
    expect({
  ...
  it("ogni operazione è avviabile da almeno un ruolo, e i tre ruoli insieme le coprono tutte", () => {
    ...
    const raggiungibili = new Set(
      RUOLI.flatMap((ruolo) => registry.getForRole(ruolo).map((descrittore) => descrittore.code)),
    );
  ...
  it.each(OPERATION_CODES)(
    ...
    (code) => {
      ...
      const descrittori = RUOLI.flatMap((ruolo) => registry.getForRole(ruolo)).filter(
        (descrittore) => descrittore.code === code,
      );
  ...
  it("un codice fuori catalogo non risolve su un agente di ripiego", () => {
    ...
    expect(() => registry.getAgent("DOCS_CHANGELOG" as never)).toThrow("Unknown OperationCode");
  });
});
