...
describe('GithubWriteService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        GithubWriteService,
        { provide: GithubClientService, useValue: githubClient },
      ],
    }).compile();
  ...
  it('creates a branch, commits the patched file, opens a PR, and returns its URL', async () => {
    ...
    const url = await service.openPullRequestForProposal(
      'token',
      'owner',
      'repo',
      'main',
    ...
    expect(githubClient.resolveRefToSha).toHaveBeenCalledWith(
      'token',
      'owner',
      'repo',
      'main',
    ...
    const createRefCall = mockRequest.mock.calls[0] as [
      string,
      { ref: string; sha: string },
    ];
    expect(createRefCall[0]).toBe('POST /repos/{owner}/{repo}/git/refs');
    expect(createRefCall[1].ref).toMatch(
      /^refs\/heads\/codeguardian\/docs-readme\/[0-9a-f]{8}$/,
    );
    ...
    expect(putContentsCall[0]).toBe(
      'PUT /repos/{owner}/{repo}/contents/{path}',
    );
    expect(putContentsCall[1].sha).toBe('file-sha');
    expect(Buffer.from(putContentsCall[1].content, 'base64').toString()).toBe(
      'new\n',
    );

    // PR: head is the branch just created, base is the original branch.
    const createPrCall = mockRequest.mock.calls[2] as [
      string,
      { base: string; head: string },
    ];
    expect(createPrCall[0]).toBe('POST /repos/{owner}/{repo}/pulls');
    expect(createPrCall[1].base).toBe('main');
    expect(createPrCall[1].head).toBe(
      createRefCall[1].ref.replace('refs/heads/', ''),
    );
  });
  ...
  it('treats a missing file as new content with no sha to update, when the agent proposes a brand-new file', async () => {
    ...
    await service.openPullRequestForProposal(
      'token',
      'owner',
      'repo',
      'main',
  ...
  it('throws PR_CREATION_FAILED when GitHub refuses the write with 403', async () => {
    ...
    await expect(
      service.openPullRequestForProposal(
        'token',
        'owner',
        'repo',
        'main',
