...
describe('TU_14 (RS.3) — nessun metodo di GithubClientService scrive su GitHub', () => {
  ...
  const PREFISSI_DI_SCRITTURA = [
    'create', 'update', 'delete', 'remove', 'add', 'insert', 'post', 'put',
    'patch', 'write', 'set', 'save', 'merge', 'push', 'upload', 'send',
    'submit', 'dispatch', 'apply', 'close', 'reopen', 'assign', 'comment',
  ];

  const sorgente = readFileSync(
    resolve(__dirname, 'github-client.service.ts'),
    'utf8',
  );
  ...
  const privati = new Set(
    [...sorgente.matchAll(/^\s*private\s+(?:async\s+)?(\w+)\s*[(<]/gm)].map(
      (m) => m[1],
    ),
  );
