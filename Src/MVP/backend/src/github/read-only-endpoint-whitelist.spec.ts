...
describe("isReadOnlyEndpointAllowed", () => {
  ...
  it("rejects a write route", () => {
    expect(isReadOnlyEndpointAllowed("POST /repos/{owner}/{repo}/pulls")).toBe(false);
  });

  it("rejects backend-direct routes that are not part of the facade", () => {
    expect(isReadOnlyEndpointAllowed("GET /repos/{owner}/{repo}/compare/{basehead}")).toBe(false);
