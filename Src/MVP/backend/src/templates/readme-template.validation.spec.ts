...
import { MAX_TEMPLATE_BYTES, validaTemplateReadme } from "./readme-template.validation";
...
describe("validaTemplateReadme", () => {
  ...
  const intestazionePng =
    String.fromCharCode(0x89) + "PNG" + String.fromCharCode(0x0d, 0x0a, 0x1a, 0x0a);
  ...
  it("accepts an uppercase extension — .MD is still Markdown", () => {
    expect(validaTemplateReadme("TEMPLATE.MD", contenutoValido).valido).toBe(true);
  });
  ...
  it("rejects a name that merely contains .md without ending in it", () => {
    ...
    expect(validaTemplateReadme("template.md.exe", contenutoValido).valido).toBe(false);
  });
  ...
  it("rejects a Windows-style path too", () => {
    expect(validaTemplateReadme("cartella\\template.md", contenutoValido).valido).toBe(false);
  });
  ...
  it("rejects a binary file renamed .md", () => {
    ...
    const esito = validaTemplateReadme("template.md", `${intestazionePng}testo`);
  ...
  it("keeps accepting tabs and newlines, which are legitimate in Markdown", () => {
    expect(validaTemplateReadme("t.md", "# Titolo\r\n\t- voce\n\n## Altro\n").valido).toBe(true);
  });

  it("rejects a template past the size cap", () => {
    const esito = validaTemplateReadme("t.md", "a".repeat(MAX_TEMPLATE_BYTES + 1));
  ...
  it("accepts a template exactly on the size cap", () => {
    expect(validaTemplateReadme("t.md", "a".repeat(MAX_TEMPLATE_BYTES)).valido).toBe(true);
  });
