...
describe("TU_19 (RF.46) — transizioni di stato ammesse della Task", () => {
  const STATI: TaskStatus[] = ["PENDING", "RUNNING", "COMPLETED", "FAILED", "CANCELLED"];
  ...
  it("accetta tutte e sole le transizioni previste, su tutte le venticinque coppie", () => {
    ...
    const effettive = Object.fromEntries(STATI.map((da) => [da, accettatiDa(da)]));
  ...
  it("da RUNNING ammette COMPLETED, FAILED e CANCELLED, e nient'altro", () => {
    expect(accettatiDa("RUNNING")).toEqual(["COMPLETED", "FAILED", "CANCELLED"]);
  });

  it.each(TERMINALI)("da %s, stato terminale, non ammette alcuna uscita", (terminale) => {
