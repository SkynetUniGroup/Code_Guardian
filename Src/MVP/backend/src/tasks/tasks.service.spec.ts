...
describe("TasksService", () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        // Segnalazione della cancellazione al servizio agenti (chiave Redis):
        // effetto collaterale su un altro sistema, qui finto.
        {
  ...
  describe("cancel", () => {
    ...
    it("transitions to CANCELLED with a conditional write and emits task.updated", async () => {
      ...
      // Due cose in una scrittura sola.
      //
      // Il filtro sullo stato: un documento letto un istante prima non puo'
      // decidere la corsa contro un TaskProcessor che finisce nel frattempo,
      // quindi la transizione e' espressa come condizione che valuta il
      // database al momento della scrittura.
      //
      // E l'azzeramento di pendingInput: senza, la richiesta di input resta
      // aperta su un Task terminale — submitInput() la trova valida, risponde
      // 204 e accoda un job che nessuno eseguira', e GET /tasks/:id continua a
      // dichiarare un pendingInput su cui il frontend decide se mostrare la
      // finestra di dialogo. E' la stessa transizione che compie il percorso
      // CANCEL di submitInput(), e per un periodo le due strade hanno detto di
      // essere la stessa cosa senza esserlo.
