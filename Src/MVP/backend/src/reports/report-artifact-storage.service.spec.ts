...
vi.mock("@aws-sdk/client-s3", () => ({
  // Una arrow function non è costruibile: `new S3Client(...)` falliva con
  // "is not a constructor". Serve una function classica (o una class).
...
describe("ReportArtifactStorageService", () => {
  ...
  describe("onModuleInit", () => {
    ...
    it("swallows a lifecycle-configuration failure too", async () => {
      mockSend.mockResolvedValueOnce({}).mockRejectedValueOnce(new Error("not supported"));
      const service = new ReportArtifactStorageService(makeConfig() as never);
