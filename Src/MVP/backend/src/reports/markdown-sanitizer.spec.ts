...
describe("sanitizeMarkdown", () => {
  ...
  it("strips a self-closing or attribute-bearing tag", () => {
    expect(sanitizeMarkdown('<img src="x" onerror="alert(1)">text')).toBe("text");
  });

  it("rejects a javascript: link destination but keeps the link text", () => {
    expect(sanitizeMarkdown("[click me](javascript:alert(1))")).toBe("click me");
  });

  it("rejects a data: link destination, case-insensitively", () => {
    expect(sanitizeMarkdown("[img](DATA:text/html;base64,PHNjcmlwdD4=)")).toBe("img");
  });

  it("leaves an ordinary https link untouched", () => {
    expect(sanitizeMarkdown("[docs](https://example.com)")).toBe("[docs](https://example.com)");
  });

  it("keeps a relative destination, which has no scheme to execute", () => {
    expect(sanitizeMarkdown("[src](./src/auth/login.ts)")).toBe("[src](./src/auth/login.ts)");
  ...
  it("drops the leading ! as well when an image destination is rejected", () => {
    ...
    expect(sanitizeMarkdown("![chart](data:image/svg+xml,<svg/onload=1>)")).toBe("chart");
  });

  it("leaves an unclosed destination alone instead of eating the rest of the text", () => {
    expect(sanitizeMarkdown("[oops](https://example.com and then more text")).toBe(
      "[oops](https://example.com and then more text",
    );
  });
  ...
  describe("adversarial payloads", () => {
    it("rejects a javascript: destination with two levels of nested parens", () => {
      ...
      // special.
      const output = sanitizeMarkdown("[click](javascript:alert(String.fromCharCode(88)))");
    ...
    it("rejects a javascript: destination split by a control character within the scheme keyword", () => {
      ...
      const output = sanitizeMarkdown("[click](java\tscript:alert(document.cookie))");
...
describe("sanitizeReportBody", () => {
  it("sanitizes markdown on a TextBlock", () => {
    const body: Block[] = [{ kind: "TEXT", markdown: "<b>hi</b> [x](javascript:1)" }];

    expect(sanitizeReportBody(body)).toEqual([{ kind: "TEXT", markdown: "hi x" }]);
  });

  it("sanitizes description and remediation on a FindingBlock, leaves structured fields alone", () => {
    const body: Block[] = [
      {
        kind: "FINDING",
        category: "A03:2021",
  ...
  it("lascia intatto uno snippet di remediation: e' codice, non Markdown", () => {
    const code = "db.query('SELECT * FROM t WHERE id = ?', [id]) // <-- parametrizzata";
    const body: Block[] = [
      {
        kind: "FINDING",
        category: "A03:2021",
        ...
        remediation: { kind: "SNIPPET", language: "ts", code },
      },
    ];

    expect(sanitizeReportBody(body)[0]).toMatchObject({
      remediation: { kind: "SNIPPET", language: "ts", code },
    });
  });
    ...
    const body: Block[] = [
      {
        ...
        explanation: "<i>bad</i>",
        severity: "LOW",
        remediation: { kind: "TEXT", text: "remove it" },
      },
    ];

    expect((sanitizeReportBody(body)[0] as { explanation: string }).explanation).toBe("bad");
  });
  ...
  it("sanitizes explanation on a ComplexityWarningBlock", () => {
    ...
    expect((sanitizeReportBody(body)[0] as { explanation: string }).explanation).toBe("too long");
  });
  ...
  it("sanitizes detail on a ChangelogItemBlock", () => {
    ...
    expect((sanitizeReportBody(body)[0] as { detail: string }).detail).toBe("details");
  });
...
describe("sanitizeMarkdown — other spellings of a hidden scheme", () => {
  const rejected = [
    ...
    ["&NewLine; inside the scheme keyword", "[x](java&NewLine;script:alert(1))"],
  ...
  it("keeps a mailto: destination carrying a reference in its parameters", () => {
    expect(sanitizeMarkdown("[m](mailto:dev@example.com?subject=A&amp;B)")).toBe(
      "[m](mailto:dev@example.com?subject=A&amp;B)",
    );
  });
...
describe("sanitizeMarkdown — links hiding inside another destination", () => {
  it("rejects a nested link two levels down", () => {
    ...
    expect(sanitizeMarkdown("[a](x [b](y [c](javascript:alert(1)) ) )")).toBe("[a](x [b](y c ) )");
  });

  it("rejects a nested image destination, dropping its ! with it", () => {
    expect(sanitizeMarkdown("[a](ok ![i](data:text/html,PHN2Zz4=) )")).toBe("[a](ok i )");
  });
  ...
  it("rejects a link nested inside what looks like a title", () => {
    ...
    expect(sanitizeMarkdown('[a](https://ok.example "t [b](javascript:1)")')).toBe(
      '[a](https://ok.example "t b")',
    );
  });

  it("rejects only the unsafe one when a region holds several nested links", () => {
    expect(sanitizeMarkdown("[a](x [ok](https://e.example) [no](javascript:1) )")).toBe(
      "[a](x [ok](https://e.example) no )",
    );
  });
  ...
  it("strips a javascript: image inside a label without taking the outer link with it", () => {
    ...
    expect(sanitizeMarkdown("[![img](javascript:alert(1))](https://ok.com)")).toBe(
      "[img](https://ok.com)",
    );
  });
});
...
describe("sanitizeMarkdown — autolinks are destinations too", () => {
  ...
  describe("the pointy-bracket destination form", () => {
    it("keeps a safe one, brackets included", () => {
      expect(sanitizeMarkdown("[x](<https://example.com/a>)")).toBe("[x](<https://example.com/a>)");
    });
  ...
  it("still strips raw HTML that merely resembles an autolink", () => {
    ...
    expect(sanitizeMarkdown('a <a href="https://e.example">b</a> c')).toBe("a b c");
    expect(sanitizeMarkdown('<img src="x" onerror="alert(1)">t')).toBe("t");
  });

  it("leaves a lone angle bracket in prose alone", () => {
    expect(sanitizeMarkdown("use a < b and c > d here")).toBe("use a < b and c > d here");
  ...
  it("judges an autolink nested inside another link’s destination", () => {
    ...
    expect(sanitizeMarkdown("[a](./ok <javascript:alert(1)> )")).toBe("[a](./ok  )");
...
describe("sanitizeMarkdown — routes that used to skip the scheme check", () => {
  describe("the label is examined too", () => {
    ...
    it("removes a data: autolink from a label, keeping the words around it", () => {
      expect(sanitizeMarkdown("[see <data:text/html,PHN2Zz4=> here](https://ok.com)")).toBe(
        "[see  here](https://ok.com)",
      );
    });
    ...
    it("keeps a safe autolink in a label untouched", () => {
      expect(sanitizeMarkdown("[see <https://ok.com> here](https://ok.com)")).toBe(
        "[see <https://ok.com> here](https://ok.com)",
      );
    });
  });
  ...
  describe("the label's brackets are matched, not scanned to the first `]`", () => {
    ...
    it("rejects a javascript: destination behind a footnote-style label", () => {
      expect(sanitizeMarkdown("[see [1] here](javascript:alert(1))")).toBe("see [1] here");
    });

    it("rejects the outer destination while keeping a safe inner link", () => {
      expect(sanitizeMarkdown("[a [b](https://ok.com) c](javascript:alert(1))")).toBe(
        "a [b](https://ok.com) c",
      );
    });
    ...
    it("keeps a bracketed label whose destination is safe", () => {
      ...
      expect(sanitizeMarkdown("[nota [2]](https://ok.com)")).toBe("[nota [2]](https://ok.com)");
      ...
      expect(sanitizeMarkdown("[[1](https://ref.example)]")).toBe("[\\[1](https://ref.example)]");
    });
  });
  ...
  describe("nothing recombines in the output", () => {
    ...
    it("does not leave a live link behind after rejecting an inner one", () => {
      expect(sanitizeMarkdown("[[lbl](javascript&#106;x)](javascript:alert(1))")).toBe("lbl");
    });

    it("does not leave one behind when the stray bracket comes from a destination", () => {
      expect(sanitizeMarkdown("[a](https://ok.example [[)](data:alert(1)) )")).toBe(
        "a](data:alert(1)) )",
      );
    });
    ...
    it("escapes a label bracket that nothing closes", () => {
      ...
      // not.
      expect(sanitizeMarkdown("[[![alt](<data:alert(1))](x)](javascript:alert(1))")).not.toMatch(
        /\]\(javascript:/,
      );
    });
  ...
  describe("character references of any length", () => {
    ...
    it("rejects a padded data: reference the same way", () => {
      expect(sanitizeMarkdown("[x](&#000000100;ata:text/html,PHN2Zz4=)")).toBe("x");
    });
    ...
    it("still keeps a query string, however long its ampersands", () => {
      ...
      expect(sanitizeMarkdown("[q](https://ok.com/s?a=1&amp;b=2&amp;c=3)")).toBe(
        "[q](https://ok.com/s?a=1&amp;b=2&amp;c=3)",
      );
    });
  });
  ...
  describe("deeply nested input is bounded", () => {
    ...
    it("still rejects an unsafe destination buried under deep nesting", () => {
      ...
      // with it.
      expect(sanitizeMarkdown(nest(40).replace("x", "javascript:alert(1)"))).not.toMatch(
        /javascript:/,
      );
    });
    ...
    it("leaves nothing linkable behind once past the ceiling", () => {
      ...
      const output = sanitizeMarkdown(nest(200).replace("x", "javascript:alert(1)"));
...
describe("sanitizeMarkdown — raw HTML that is not a tag", () => {
  it("removes an HTML comment and what it hides", () => {
    expect(sanitizeMarkdown('testo <!-- <a href="javascript:alert(1)">x</a> --> qui')).toBe(
      "testo  qui",
    );
  });
  ...
  it("removes a CDATA section", () => {
    expect(sanitizeMarkdown('<![CDATA[<a href="javascript:alert(1)">x</a>]]>')).toBe("");
  });
  ...
  it("defuses an unclosed comment without eating the text after it", () => {
    ...
    // the opener goes.
    expect(sanitizeMarkdown("prima <!-- e tutto il resto")).toBe("prima  e tutto il resto");
  });

  it("leaves ordinary prose containing < and ! alone", () => {
    expect(sanitizeMarkdown("if (a < b) then! else")).toBe("if (a < b) then! else");
