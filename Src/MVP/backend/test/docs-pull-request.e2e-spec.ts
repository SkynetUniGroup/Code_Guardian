...
describe('TI_12 (RF.82, RF.63) / TI_13 (RF.72) — apertura automatica della Pull Request', () => {
  ...
  it('un collegamento alla PR gia\' valorizzato sopravvive fino all\'API', async () => {
    ...
    const report = await reportLetto(utente.token, conclusa.reportId);
    expect(report.proposal.pullRequestUrl).toBe(
      'https://github.com/OWASP/NodeGoat/pull/7',
    );
  }, 180_000);
  ...
  it.failing(
    'TI_12 — DIFETTO APERTO: ricevuta la Proposal, il backend non apre alcuna Pull Request',
    async () => {
      ...
      expect(
        ambiente.scritturaGithub.openPullRequestForProposal,
  ...
  it.failing(
    'TI_12 — DIFETTO APERTO: il Report non porta il collegamento alla PR aperta dal backend',
    async () => {
  ...
  it.failing(
    'TI_13 — DIFETTO APERTO: il rifiuto di GitHub non porta la task a FAILED con PR_CREATION_FAILED',
    async () => {
      ...
      expect(persistito?.proposal?.diffUnified).toContain(
        '+Guida introduttiva.',
      );
    },
    180_000,
