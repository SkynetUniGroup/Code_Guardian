...
describe('TI_15 (RF.66, RV.6) — tetto mensile prima dell\'accodamento', () => {
  ...
  let tetto: number;
  let spiaAccodamento: jest.SpyInstance;
  ...
  beforeEach(() => {
    ...
    spiaAccodamento = jest.spyOn(ambiente.coda, 'addBulk');
  });
  ...
  it('il rifiuto e\' totale: nessuna operazione del batch viene persistita', async () => {
    ...
    expect(await ambiente.taskModel.countDocuments({ userId: utente.userId }))
      .toBe(0);
    expect(spiaAccodamento).not.toHaveBeenCalled();
  }, 120_000);
