...
describe("TU_27 (RV.16) — documento OpenAPI del backend", () => {
    ...
    return strati
      ...
      .flatMap((strato) =>
        Object.keys(strato.route!.methods)
          ...
          .map((metodo) => `${metodo} ${strato.route!.path.replace(/:([A-Za-z0-9_]+)/g, "{$1}")}`),
      );
  }
  ...
  beforeAll(async () => {
    ...
    const config = new DocumentBuilder()
      ...
      .addBearerAuth()
      .build();
    documento = SwaggerModule.createDocument(ambiente.app, config) as unknown as Record<
      string,
      unknown
    >;
  }, 60_000);
  ...
  it("ogni parametro di percorso e' documentato come obbligatorio e tipizzato", () => {
    const mancanti = operazioni().flatMap(({ percorso, metodo, op }) => {
      ...
      return segnaposto
        .filter((nome) => {
          const parametro = (op.parameters ?? []).find((p) => p.name === nome && p.in === "path");
  ...
  it("ogni endpoint pubblico servito dall'applicazione compare nel documento", () => {
    ...
    const documentate = operazioni().map(({ metodo, percorso }) => `${metodo} ${percorso}`);
  ...
  it.fails("RV.16 — DIFETTO APERTO: gli schemi dei corpi di richiesta sono vuoti", () => {
  ...
  it.fails("RV.16 — DIFETTO APERTO: nessuna operazione documenta lo schema della propria risposta", () => {
    ...
    const senzaSchema = operazioni()
      .filter(({ op }) => Object.values(op.responses ?? {}).every((risposta) => !risposta.content))
      .map(({ metodo, percorso }) => `${metodo} ${percorso}`);
  ...
  it("RV.16 — i parametri di query sono documentati", () => {
    ...
    const esportazione = operazioni().find(
      ({ percorso, metodo }) => percorso.endsWith("/export") && metodo === "get",
    );
