...
describe('TI_09 / TI_10 — sospensione, ripresa e annullamento di CHANGELOG_BUSINESS', () => {
  ...
  async function attendiSospensione(
    taskId: string,
    kind: string,
  ...
  it('TI_09 — attraversa le tre sospensioni nell\'ordine previsto e arriva a COMPLETED', async () => {
    ...
    const terzaFermata = await attendiSospensione(
      taskId,
  ...
  it.each([
    ...
    async (fermata) => {
      ...
      expect((await ambiente.taskModel.findById(taskId))!.status).toBe(
        'CANCELLED',
      );
    },
    180_000,
  ...
  it.failing(
    'TI_10 — DIFETTO APERTO: /cancel non azzera pendingInput, e la task continua ad accettare risposte',
    async () => {
