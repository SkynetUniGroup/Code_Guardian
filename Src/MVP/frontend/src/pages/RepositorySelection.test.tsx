...
beforeEach(() => {
  ...
  getOperationsMock
    .mockReset()
    .mockResolvedValue([{ code: "SECURITY_OWASP", name: "Scansione OWASP" }]);
...
describe("RepositorySelection", () => {
  it("precompila il form con i dati salvati nello store (persistenza tra navigazioni)", () => {
    useAppStore.getState().setFormData({
      repoOwner: "skynet",
  ...
  it("mostra il pulsante \"Carica operazioni\" finche' la select non e' stata popolata", () => {
    render(<RepositorySelection />);
    expect(
      screen.getByRole("button", { name: /Carica operazioni disponibili/i }),
    ).toBeInTheDocument();
  ...
  it("in caso di errore con messaggio dal backend lo mostra in alert e non naviga", async () => {
    const user = userEvent.setup();
    createContextMock.mockRejectedValueOnce({
      response: { data: { message: "Repository non raggiungibile" } },
    });
    render(<RepositorySelection />);
    ...
    await waitFor(() =>
      expect(window.alert).toHaveBeenCalledWith(
        expect.stringContaining("Repository non raggiungibile"),
      ),
    );
