...
describe("TemplatePage", () => {
  ...
  it("senza template personalizzato dichiara che è in uso il default", async () => {
    ...
    expect(screen.getByText(/Nessun template personalizzato/)).toBeInTheDocument();
  });
  ...
  it("carica il file scelto inviando nome e contenuto", async () => {
    ...
    await user.upload(screen.getByLabelText(/Carica un template/), fileMarkdown("nuovo.md"));
  ...
  it("mostra il motivo per cui il backend ha rifiutato il file", async () => {
    ...
    await user.upload(screen.getByLabelText(/Carica un template/), fileMarkdown("note.txt"));

    expect(await screen.findByRole("alert")).toHaveTextContent("estensione .md");
  });
  ...
  it("ricade su message quando la risposta non porta details", async () => {
    ...
    await user.upload(screen.getByLabelText(/Carica un template/), fileMarkdown("vuoto.md", ""));
  ...
  it("dopo un rifiuto lascia lo stato precedente invariato", async () => {
    ...
    await user.upload(screen.getByLabelText(/Carica un template/), fileMarkdown("rotto.txt"));
  ...
  it("rimuove il template e dichiara il ritorno al modello di default", async () => {
    ...
    await waitFor(() => expect(deleteMock).toHaveBeenCalledWith("/templates/readme"));
  ...
  it("non offre la rimozione quando non c'è nulla da rimuovere", async () => {
    ...
    expect(screen.queryByRole("button", { name: /Rimuovi template/ })).not.toBeInTheDocument();
  });
