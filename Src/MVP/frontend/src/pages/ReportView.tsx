  ...
  return (
    <div className="mt-8 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <div className="bg-gray-50 border-b border-gray-200 px-5 py-4 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold text-[#2a2a2a]">Proposta di modifica (Diff)</h2>
        ...
        <pre className="text-sm font-mono leading-tight py-4">
          {lines.map((line: string, index: number) => {
            ...
            if (line.startsWith("+") && !line.startsWith("+++")) {
              ...
              lineClass += " font-bold text-[#2a2a2a] pb-2";
            } else {
      ...
      <div className="bg-blue-50 px-5 py-3 border-t border-blue-100 flex items-start gap-3">
        ...
        <p className="text-sm text-blue-800">
          <strong>Azione richiesta:</strong> L'applicazione di questa proposta al repository è un
          atto umano e resta a carico dell'utente. Il sistema Code Guardian opera in sola lettura.
        </p>
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-3 text-gray-500">Caricamento report...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-8">
      <div className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-[#2a2a2a]">Analisi: {report.operation}</h1>
      ...
      {report.summary && (
        <div className="mb-8 p-5 bg-white border border-gray-200 shadow-sm rounded-xl">
          <h2 className="text-lg font-semibold text-[#2a2a2a] mb-2">Riassunto</h2>
      ...
      {report.error && (
        <div className="mb-8 p-5 bg-red-50 border border-red-200 rounded-xl shadow-sm">
          ...
          <div className="text-sm font-mono bg-white p-3 rounded border border-red-100 text-red-900 mb-2">
            <strong>Codice:</strong> {report.error.code} | <strong>Fase:</strong>{" "}
            {report.error.stage}
          </div>
      ...
      {report.body && report.body.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xl font-bold text-[#2a2a2a] mb-4">Dettagli</h2>
