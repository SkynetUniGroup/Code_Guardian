...
test.describe("Percorsi d'errore nella selezione del repository", () => {
  ...
  test("RF.20 — repository inesistente: mostra un errore chiaro e non avvia la task", async ({
    page,
  }) => {
  ...
  test("RF.31 — ambito che supera il limite di 100 file: mostra l'errore dimensionale", async ({
    page,
  }) => {
    await loginAndReachRepoForm(page);
