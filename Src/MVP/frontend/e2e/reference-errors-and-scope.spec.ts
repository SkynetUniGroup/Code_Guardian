...
test.describe("Riferimento e ambito", () => {
  ...
  test("RF.21/RF.22 — ref (branch o commit) inesistente: errore esplicito, nessun avvio", async ({
    page,
  }) => {
    await login(page);
  ...
  test("RF.26/UC16.1 — nessuno scope selezionato (intero repository): completa con successo se sotto il limite file", async ({
    page,
  }) => {
    await login(page);
  ...
  test("RF.27 — ambito ristretto a un singolo file (non una directory): funziona", async ({
    page,
  }) => {
    await login(page);
    await page.getByPlaceholder("skynetunigroup").fill("IlGranz");
  ...
  test("RF.30/UC18 — ambito (directory) inesistente in un repository valido: errore esplicito", async ({
    page,
  }) => {
    await login(page);
    await page.getByPlaceholder("skynetunigroup").fill("IlGranz");
