...
def _firma_attesa(endpoint: str, corpo: str, timestamp: str) -> str:
    ...
    Il path firmato e' quello che il backend vede in `request.path`, cioe'
    comprensivo del prefisso globale che main.ts installa con
    setGlobalPrefix('api/v1'): firmare il solo endpoint interno produrrebbe
    un HMAC che InternalAuthGuard rifiuta.
    """
    body_hash = hashlib.sha256(corpo.encode('utf-8')).hexdigest()
    messaggio = f'{timestamp}:POST:{PREFISSO}{endpoint}:{body_hash}'.encode('utf-8')
