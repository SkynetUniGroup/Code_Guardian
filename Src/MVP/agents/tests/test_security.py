...
from tests.conftest import FakeContextRef, FakeToolset
...
def test_parse_output_keeps_info_severity():
    """INFO e' una gravita' a se': viene normalizzata, non ricondotta a LOW.

    Nel modello precedente INFO non esisteva e veniva schiacciata su LOW.
    Ora e' il gradino piu' basso di Severity, dichiarato allo stesso modo in
    shared/src/types.ts, in models.py e dal badge del frontend: ricondurla a
    LOW farebbe sparire la distinzione lungo tutta la catena.
    """
...
def test_tu11_info_severity_is_ranked_last():
    """INFO resta INFO e finisce in fondo all'ordinamento, non scartato."""
    raw = _owasp_output([
    ...
    assert [b.severity for b in blocks] == ['MEDIUM', 'INFO']
