...
def _toolset_finto(monkeypatch):
    """Rimpiazza il toolset che il grafo si ricostruisce da solo.

    In questa versione il toolset non sta piu' dentro AgentState -- ci teneva
    INTERNAL_SHARED_SECRET, che il checkpointer riversava su MongoDB in chiaro
    a ogni nodo -- ma viene ricreato al volo da AgentGraph._toolset(). I test
    non possono piu' iniettarlo costruendo lo stato: lo sostituiscono qui.

    E' autouse perche' un GitHubToolset vero aprirebbe connessioni HTTP verso
    un backend che in test non esiste; nessun test legge questo oggetto, gli
    serve solo che le chiamate del grafo non escano dal processo.
    """
    try:
        from src.graph import AgentGraph
    except ImportError:  # moduli di test che non toccano il grafo
        return
    monkeypatch.setattr(
        AgentGraph, '_toolset', staticmethod(lambda st: FakeToolset()), raising=False
    )
