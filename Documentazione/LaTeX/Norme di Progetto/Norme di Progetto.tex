...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Scopo: sviluppare e testare una nuova funzionalità isolatamente.
...
\begin{itemize}
    ...
    \item \textit{Workflow principali}: \texttt{.github/workflows/ci.yml}.
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item Fasi:
            \begin{enumerate}
                \item Checkout codice.
                \item Setup Node.js/Python.
                \item Installazione dipendenze (\texttt{pnpm install}).
                \item \textit{Linting} (\texttt{pnpm lint} con \textit{Biome}).
                \item Test unitari (\texttt{pnpm test:unit}).
                \item Test di integrazione (\texttt{pnpm test:integration}).
                \item \textit{Build} (\texttt{pnpm build}).
            \end{enumerate}
            \item Requisiti per il \textit{merge}: tutte le fasi devono passare.
        \end{itemize}
        \item Deploy Staging (\texttt{deploy-staging.yml})
        \begin{itemize}
            \item Trigger: push su \texttt{develop}.
            \item Fasi: 1-5) Stesse di Pull Request Check. 6) Deploy su GitHub Pages.
...
\newpage
\paragraph{Script \textit{npm} (in \textit{.json})}
\begin{verbatim}
{
  "scripts": {
    "dev": "vite",
...
\paragraph{Framework}
\begin{itemize}
    \item \textit{TypeScript}: \textit{Vitest}. Configurazione in \texttt{vite.config.ts}.
    \item \textit{Python}: \textit{pytest}. Configurazione in \texttt{pytest.ini}.
\end{itemize}
...
\begin{itemize}
    \item Esecuzione in \textit{CI/CD} con \textit{GitHub Actions} ad ogni \textit{push} su \texttt{main}/ \texttt{develop}.
    \item Script per test locali:
    \begin{itemize}
        ...
        "scripts": {
            ...
            "test:unit:coverage": "pytest --cov=src tests/unit"
         }
        \end{verbatim}
    \end{itemize}
\end{itemize}
...
\paragraph{Ambiente}
\begin{itemize}
    \item \textit{Docker} per servizi esterni (\textit{MongoDB}, \textit{API mock}).
...
\begin{itemize}
    \item \textit{Fixture}: file in \texttt{tests/fixtures/}.
...
\begin{itemize}
    ...
    \item \textit{CI}: in \textit{workflow} separato dai test d'unità.
\end{itemize}
