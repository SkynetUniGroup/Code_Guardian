...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item la definizione di uno o più \textit{template} documentali da utilizzare durante la redazione dei documenti, al fine di standardizzarli.
    \end{itemize}
...
Su ogni documento possono lavorare più membri contemporaneamente. Per ridurre il numero di \textit{conflitti merge} da risolvere, ogni membro dovrebbe effettuare un \textit{commit} nel \textit{repository} del progetto al completamento di ogni piccola unità di lavoro

I documenti vengono pubblicati sul sito del gruppo. Non tutti i documenti ci sono pubblicati, ma solo quelli relativi alla milestone principale corrente (candidatura al capitolato, RTB, PB). Tutti i documenti sono comunque presenti nel \textit{repository} \textit{Git} del progetto.\\
...
\begin{itemize}
    ...
    \item \textbf{Lettere di presentazione} - documenti redatti in occasione degli eventi principali dell'insegnamento (candidatura al capitolato, RTB, PB). Tali lettere non sono soggette a manutenzione successiva dopo il loro invio al \textbf{Committente} via email.
\end{itemize}
...
\begin{itemize}
    \item \textit{Overleaf} - editor \LaTeX{} collaborativo basato su \textbf{\glo{cloud}{cloud}}, utilizzato per la redazione, la formattazione e l'affinamento finale dei documenti

    \item \textit{TeXPage} - editor \LaTeX{} collaborativo basato su \textbf{\glo{cloud}{cloud}}, utilizzato in alternativa a \textit{Overleaf} per la compilazione di documenti di grandi dimensioni, come ad esempio l'\textbf{Analisi dei Requisiti};

    \item {[Nome Strumento di verifica ortografica]} - utilizzato per individuare errori ortografici e tipografici nei documenti prodotti

    \item {[Nome Strumento per il calcolo dell'Indice di Gulpease]} - utilizzato per valutare la leggibilità dei documenti redatti in lingua italiana
\end{itemize}
...
\textbf{Creazione} - La configurazione dell'infrastruttura va pianificata e documentata.
 ...
 \textit{Overleaf} - Un editor \textit{LaTeX} collaborativo basato su \textbf{\glo{cloud}{cloud}}. Utilizzato per la formattazione precisa dei file di documentazione e per il loro affinamento finale.
 ...
 \textit{Google Sheets} - Un\textquotesingle applicazione per fogli di calcolo basata sul cloud che consente agli utenti di collaborare sui dati in tempo reale.
