...
\usepackage{hyperref}
\usepackage{enumitem}\usepackage{textcomp}
...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            ...
            \textbf{Verifica} & \textit{Marco Barbiero, Dario Pirrone, Samuele Bertin, Edoardo Granziero, Andrea Fistarol} \\ \hline
            \textbf{Approvazione} & \textit{Samuele Bertin, Dario Pirrone, Andrea Fistarol, Edoardo Granziero, Marco Barbiero} \\ \hline
...
1.3.0 & 2026/09/22 & Controllo ortografia e impaginazione; aggiornamenti minori & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{-} \\ \hline
1.2.0 & 2026/08/31 & Aggiornamento della struttura della \textit{repository} e dei \textit{workflow} & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{Edoardo \\Granziero} \\ \hline
1.1.0 & 2026/08/21 & Ampliate e riviste le sezioni Codifica, Struttura Repository, Test di Unità e Test di Integrazione; creata la sezione Strumenti di Build & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{Marco \\Barbiero} \\ \hline
1.0.1 & 2026/08/21 & Aggiunta versione al riferimento del glossario e aggiunta data di accesso alle risorse web di riferimento & \makecell[l]{Marco \\Barbiero} & \makecell[l]{Dario \\Pirrone} \\ \hline
1.0.0 & 2026/08/13 & Approvazione del documento per revisione RTB & \makecell[l]{Andrea \\Fistarol} & -- \\ \hline
0.11.0 & 2026/08/07 & Riscrittura della sezione 3.2 & \makecell[l]{Marco \\Barbiero} & \makecell[l]{Edoardo \\Granziero} \\ \hline
0.10.0 & 2026/07/20 & Riscrittura delle sottosezioni Verifica e Validazione & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Andrea \\Fistarol} \\ \hline
0.9.0 & 2026/07/15 & Riscrittura della sottosezione Documentazione & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Andrea \\Fistarol} \\ \hline
0.8.0 & 2026/07/14 & Scrittura della sezione ``Metriche per la Qualità'' (5) & \makecell[l]{Dario \\Pirrone} & \makecell[l]{Samuele \\Bertin} \\ \hline
0.7.0 & 2026/07/07 & Verifica dettagliata e correzione del documento intero & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Edoardo \\Granziero} \\ \hline
0.6.0 & 2026/05/23 & Scrittura della sottosezione ``Sviluppo'' (2.2) & \makecell[l]{Dario \\Pirrone} & \makecell[l]{Samuele \\Bertin} \\ \hline
0.5.0 & 2026/05/13 & Scrittura della sezione ``Processi di Supporto'' & \makecell[l]{Edoardo \\Granziero} & \makecell[l]{Dario \\Pirrone} \\ \hline
0.4.0 & 2026/05/12 & Scrittura della sottosezione ``Fornitura'' & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{Dario \\Pirrone} \\ \hline
0.3.0 & 2026/04/27 & Finita la scrittura della sezione ``Processi Organizzativi'' & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Marco \\Barbiero} \\ \hline
0.2.0 & 2026/04/20 & Scrittura delle sezioni ``Gestione di Processi'' e ``Infrastruttura di Supporto'' & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Marco \\Barbiero} \\ \hline
0.1.0 & 2026/04/10 & Strutturazione iniziale del documento; scrittura di ``Introduzione'' & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Marco \\Barbiero} \\ \hline
\end{xltabular}
}
...
Il gruppo decide collegialmente a quale \textit{capitolato} candidarsi, assegnando a ognuno un punteggio secondo parametri prestabiliti. Per ogni \textit{capitolato}, deve essere data motivazione per l'accettazione o il rifiuto.

\subsubsection{Preparazione della risposta}
Il \textit{capitolato} selezionato viene esplorato ulteriormente per raffinare la candidatura e massimizzare le possibilità di successo della stessa. Va realizzato il \textit{\glo{Preventivo}{preventivo}} costi e assunzione impegni, secondo le modalità e i vincoli specificati nel Regolamento del progetto, e va preparata la Lettera di Presentazione.
...
\begin{itemize}
    ...
    \item \textbf{\glo{Trigger}{trigger}}: 
...
\subparagraph{\textit{\glo{TypeScript}{typescript}}}
\begin{itemize}
    \item File: \texttt{kebab-case}
    \item Variabili e Funzioni: \texttt{camelCase}
    \item Classi, Interfacce e Tipi: \texttt{PascalCase}
...
\begin{itemize}
    ...
    \item \textit{\glo{Linter}{linter} e formatter}: \textit{Biome} (\textit{TS}), \textit{Ruff} (\textit{Python}).
    \item Analisi Statica: \textit{SonarQube}, \textit{semgrep}.
\end{itemize}
...
I documenti vengono pubblicati sul \href{https://skynetunigroup.github.io/Code_Guardian/}{sito} del gruppo. Non tutti i documenti vi sono pubblicati, ma solo quelli relativi alla milestone principale corrente (candidatura al capitolato, RTB, PB). Tutti i documenti sono comunque presenti nel \textit{repository} \textit{Git} del progetto.\\
...
L'organizzazione del \textit{repository} si ispira al modello \textit{\glo{Git Flow}{git-flow}} ed è così articolata:
...
Viene adottato lo standard \textit{\glo{Conventional Commits}{conventional-commits}}, che prescrive la seguente forma per i \textit{commit}:
\begin{center}
...
\begin{itemize}
    \item \textit{pnpm}: comandi \texttt{pnpm install}, \texttt{pnpm add -d [package]}, \texttt{pnpm remove -d [package]}.
        ...
        \end{itemize}
    \item \textit{\glo{Lockfile}{lockfile}}: \texttt{pnpm-lock.yaml} da "committare". \textbf{Mai} modificarlo manualmente.
    \item \textit{Workspace}: struttura monorepo con \texttt{pnpm-workspace.yaml}.
\end{itemize}

\paragraph{\textit{Bundling} e \textit{Dev Server}}
\begin{itemize}
    \item \textit{\glo{Vite}{vite}}: configurazione in \texttt{vite.config.ts}.
    \begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}[label=\arabic*.]
                ...
                \item \textit{\glo{Linting}{linter}} \textit{Python}.
...
\paragraph{Framework}
\begin{itemize}
    \item \textit{TypeScript}: \textit{\glo{Vitest}{vitest}}. Configurazione in \texttt{vite.config.ts}.
    \item \textit{Python}: \textit{\glo{pytest}{pytest}}. Configurazione in \texttt{pytest.ini}.
\end{itemize}

\paragraph{Copertura Minima}
\begin{itemize}
    \item 75\% globale.
    \item Misurata con \textit{V8 (Vitest)} e \texttt{pytest-cov}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Nome dei test:
    \begin{itemize}
        \item \textit{TypeScript}: \verb+{describe("[modulo]", ()=>{it("[scenario]", ()=>{...})}+
        \item \textit{Python}: \texttt{test\_[funzione]\_[scenario]}.
    \end{itemize}
    \item Struttura: \textit{\glo{AAA pattern}{aaa-pattern}} (Arrange-Act-Assert).
\end{itemize}
...
\begin{itemize}
    \item \textit{TypeScript}: integrato in \textit{Vitest} (es. \texttt{vi.fn()}, \texttt{vi.mock()}).
    \item \textit{Python}: \texttt{unittest.mock}
\end{itemize}

\paragraph{Automazione}
\begin{itemize}
    \item Esecuzione con \texttt{pnpm test:unit} in \textit{CI/CD} tramite \textit{GitHub Actions} ad ogni \textit{push} (\textit{workflow} \texttt{deploy-staging.yml}) o \textit{Pull Request} (\textit{workflow} \texttt{pr-check.yml}) su \texttt{develop}.
\end{itemize}
...
\paragraph{Framework} \textit{Vitest} e \textit{pytest}.

\paragraph{Ambiente}
\begin{itemize}
    \item \textit{\glo{Docker Compose}{docker-compose}} per servizi esterni (\textit{MongoDB}, \textit{API mock}).
    \item Variabili d'ambiente: prefisso \texttt{TEST\_}.
\end{itemize}
...
\begin{itemize}
    \item \textit{\glo{Fixture}{fixture}}: file in \texttt{tests/fixtures/}.
    \item Pulizia: reset DB tra test (es. \texttt{pytest-mongodb} \textit{fixture}).
\end{itemize}

\paragraph{Esecuzione}
\begin{itemize}
    \item Locale: \texttt{pnpm test:integration}
    \item \textit{CI}: Esecuzione in \textit{CI/CD} con \textit{GitHub Actions} come parte dei \textit{workflow} \texttt{pr-check.yml} (\textit{Pull Request}) e \texttt{deploy-staging.yml} (\textit{push} su \texttt{develop}), insieme ai test unitari.
\end{itemize}
...
\begin{itemize}
\item
  la posta elettronica - per inviare email al \textbf{Proponente}, ai collaboratori dell'azienda \textbf{Proponente} e ai \textbf{Committenti}
\item
...
\textbf{Creazione} - La configurazione dell'infrastruttura va pianificata e documentata.
 ...
 \textit{Discord} - Un'app di comunicazione che combina messaggistica istantanea, videochiamate e chat vocale in comunità organizzate per argomento, chiamate ``server''. Utilizzato per la comunicazione interna e come piattaforma dove svolgere le nostre riunioni interne.
...
\begin{itemize}
\item
  Documentazione: doc-da-verificare, sviluppo-analisi-requisiti, sviluppo-piano-progetto, sviluppo-glossario, sviluppo-norme-progetto, sviluppo-piano-qualifica, verbali, progettazione
\item
  Coding: coding, integrazione, testing
\item
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_9 - Tasso di Errore (Error Rate) \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_10 - Tempo di Risposta Interfaccia \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_11 - Tempo di Risposta dell'Agente \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_12 - Tempo di Autenticazione \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_13 - \textit{\glo{Prompt}{prompt}} Isolation (Disaccoppiamento dei \textit{Prompt}) \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_14 - \textit{\glo{Complessità Ciclomatica}{complessità-ciclomatica}} \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_15 - \textit{Code Smell} \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_16 - Duplicazione del Codice \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_17 - Esposizione di Credenziali \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_18 - Cifratura dei Dati Sensibili \\ \hline
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MPD\_19 - Input Validation \\ \hline
