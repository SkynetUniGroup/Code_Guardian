...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            \hline
            \textbf{Versione} & \textit{0.8.1} \\ \hline
...
\begin{table}[h]
 ...
 \renewcommand{\arraystretch}{1.3}
    \begin{tabularx}{\textwidth}{|c|c|X|l|l|}
        ...
        0.8.1 & 2026/08/07 & Scrittura della sezione 3.2 & Marco Barbiero &  Edoardo Granziero \\ \hline
        0.7.1 & 2026/07/15 & Implementazione delle correzioni della verifica dettagliata & Sara Ristovic &  Edoardo Granziero \\ \hline
        0.7.0 & 2026/07/14 & Scrittura della sezione "Metriche per la Qualità" (5) & Dario Pirrone &  \\ \hline
        ...
        0.3.0 & 2026/04/27 & Finita la scrittura della sezione ``Processi Organizzativi'' & Sara Ristovic 
 & Marco Barbiero \\ \hline
        0.2.0 & 2026/04/20 & Scrittura delle sezioni ``Gestione di Processi'' e ``Infrastruttura di Supporto'' &
...
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2009/Approfondimenti/ISO_12207-1995.pdf}{Standard \textit{ISO/IEC 12207:1995}}
...
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2025/Dispense/T02.pdf}{Slide T02 dell'insegnamento Ingegneria del Software (Processi di Ciclo di Vita)}
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2025/Dispense/T03.pdf}{Slide T03 del corso Ingegeria del Software (Il Ciclo di Vita del SW)}
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2025/Dispense/T04.pdf}{Slide T04 dell'insegnamento Ingegneria del Software (Gestione di Progetto)}
\item
...
\begin{itemize}
    \item \textbf{Requirements and Technology Baseline (RTB):} questa fase è fortemente orientata allo studio di fattibilità e alla riduzione dei rischi tecnologici. Il team valuta i \textit{framework}, le librerie e gli strumenti candidati per lo sviluppo. L'attività principale consiste nella realizzazione di un \textbf{\glo{Proof of Concept (PoC)}{poc}}, un prototipo software semplificato utile a verificare la fattibilià dell'idea di progetto e l'integrazione tra le tecnologie scielte, prima della produzione su larga scala.
...
\end{itemize}
Il numero di versione di ogni documento è riportato nella tabella delle informazioni principali e nel registro delle modifiche descritti nella sezione \textit{3.1.2 Progettazione}. Lo stesso codice di versione è riportato anche nel nome dei documenti compilati pubblicati (ad esempio \texttt{norme\_di\_progetto\_v0.7.1.pdf}), così da rendere immediatamente riconoscibile la versione di ciascun artefatto.
...
\paragraph{Repository utilizzata}\mbox{}\\
...
\paragraph{Organizzazione dei rami}\mbox{}\\
L'organizzazione del \textit{repository} si ispira al modello \textit{GitHub Flow} ed è così articolata:
\begin{itemize}
    \item \textbf{\texttt{main}}: ramo principale contenente esclusivamente artefatti (codice e documentazione) approvati da almeno un \textbf{Verificatore} e dal \textbf{Responsabile}. Ogni stato del ramo \texttt{main} rappresenta una configurazione stabile del progetto;
    \item \textbf{Rami secondari}: rami creati a partire da \texttt{main} per lo sviluppo di uno specifico compito o componente. Al termine della lavorazione, le relative modifiche vengono proposte tramite \textit{Pull Request} e, una volta approvate da un \textbf{Verificatore}, integrate (\textit{merge}) nel ramo \texttt{main}. I rami secondari vengono mantenuti nel \textit{repository} anche dopo il \textit{merge}, così da conservare la tracciabilità del lavoro svolto.
\end{itemize}
Ogni \textit{\glo{commit}{commit}} deve avere un messaggio conciso e significativo che ne descriva il contenuto. Per ridurre i \textit{conflitti} in fase di \textit{merge}, ogni membro dovrebbe effettuare un \textit{commit} al completamento di ogni piccola unità di lavoro.
\paragraph{Organizzazione delle cartelle}\mbox{}\\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{\texttt{LaTeX/}}: sorgenti \LaTeX{} dei documenti, con una sotto-cartella per ciascun documento (\textbf{Analisi dei Requisiti}, \textbf{Norme di Progetto}, \textbf{Piano di Progetto}, \textbf{Piano di Qualifica}, \textbf{Glossario}) e la cartella \texttt{Template/} contenente i \textit{template} documentali;
    ...
    \end{itemize}
    \item \textbf{\texttt{Src/}}: contiene il codice sorgente dell'applicativo (agenti, \textit{backend} e \textit{frontend}) e i relativi test automatici;
    \item \textbf{\texttt{Website/}}: contiene i sorgenti del sito del gruppo, pubblicato tramite \textit{GitHub Pages};
    \item \textbf{\texttt{.github/workflows/}}: contiene i \textit{workflow} di \textit{GitHub Actions} per l'automazione del \textit{deploy} dei contenuti statici;
...
Questo processo determina se gli artefatti del progetto soddisfano i requisiti e le condizioni imposti precedentemente. {[...non abbia introdotto errori...]} Per ogni modifica apportata in documentazione, progettazione o codice, questo processo formalmente non può esegure lo stesso autore della modifica, ma un altro membro del gruppo. Inoltre, sono necessarie verifiche esterne da parte del \textbf{Proponente} per gli artefatti del progetto più importanti e del loro interesse.

\subsubsection{Verifica della documentazione}
Si verifica se la documentazione sia sufficientemente leggibile, se sia consistente, completa e se l'argomento viene affrontato corretamente e in modo sufficientemente profondo.\\
Si noti che non basta una lettura delle modifiche e la verifica solamente della loro leggibilità e che quanto scritto abbia senso. 

Le correzioni di verifica vengono riportate evidenziando le parti da correggere nel file .pdf, accompagnate da un commento breve.
...
\textbf{Inizializzazione} - Le attività e i compiti da svolgere vengono identificati individuando gruppi di requisiti correlati, che possono essere obbligatori o facoltativi. È necessario valutare attentamente la fattibilità di ogni singola attività e compito analizzando la disponibilità delle risorse necessarie per lo svolgimento.
 ...
 \textbf{Pianificazione} - È la pianificazione dell'esecuzione di attività e compiti. Il piano deve descrivere bene il compito, gli strumenti che utilizzeremo, \textit{preventivo} e consuntivo dei costi, a chi viene assegnato il compito, scadenze e rischi potenziali.
...
Ci sono 3 tipologie di riunioni in cui è necessario partecipare: interne, esterne, \textbf{diari di bordo}.

\paragraph{Riunioni Interne}\mbox{}\\
In queste riunioni partecipano esclusivamente i membri del gruppo \textbf{SkyNet} che lavorano sul progetto. Ogni membro è tenuto a partecipare.\\
...
\item
 ...
 \item
  Lo stato corrente della formazione del personale del fornitore (gruppo \textbf{SkyNet}). Se e come il \textbf{Proponente} può fare il mentore e aiutare i membri di \textbf{SkyNet} di imparare certe tecnologie richieste.
 \item
...
\begin{tabular}{|l|p{12cm}|}
\hline
\textbf{Metrica} & MP\_10 - Indice di \textit{\glo{Gulpease}{gulpease}} \\ \hline
