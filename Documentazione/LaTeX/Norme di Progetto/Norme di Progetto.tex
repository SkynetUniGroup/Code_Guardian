...
Questo processo registra le informazioni prodotte durante un processo di ciclo di vita o durante un'attività di progetto. È fondamentale per molte ragioni:
\begin{itemize}
    \item favorisce la chiarezza delle informazioni stabilite e l'assenza di ambiguità
    \item fornisce un accesso facile agli interessati
    \item aiuta la comprensione uguale di tutti i membri del gruppo
    \item definisce le regole e i processi da seguire e le metriche da rispettare
    \item descrive il prodotto software, ne spiega e ne facilita l'utilizzo
\end{itemize}
Come si può vedere dagli scopi della documentazione, ci sono vari enti interessati in vari documenti, tra cui ingegneri, sviluppatori, manager, utenti finali del prodotto software, ecc.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item l'identificazione di documenti necessari
        \item l'identificazione di tutti gli elementi principali in comune ad ogni documento
        \item la progettazione degli indici di documenti
        \item stabilizzazione di un template documentale (o più) che verrà utilizzato durante la redazione di documenti, per standardizzarli
    \end{itemize}
    
    \item \textbf{Produzione} - la redazione stessa dei documenti e la loro fornitura
    \item \textbf{Manutenzione} - l'aggiornamento dei documenti già presenti per preservarne la rilevanza e l'utilità 
\end{itemize}

\subsubsection{Progettazione}

All'inizio di ogni fase principale del progetto (candidatura al capitolato, RTB, PB) bisogna identificare quali documenti servono per concludere la milestone con successo.

Gli elementi in comune alla maggioranza dei documenti sono: 
\begin{itemize}
    \item titolo
    ...
    \item tabella delle informazioni principali sul documento:
        \begin{itemize}
            \item versione
            \item uso (interno cioè serve esclusivamente ai membri del gruppo; esterno cioè ...)
            \item destinatari (di solito i professori commitenti)
            \item redattori - i membri del gruppo che hanno contribuito nella scrittura del contenuto
            \item verifica - i membri che hanno fatto verifiche
            \item approvazione - i membri che hanno approvato il documento
        \end{itemize}
    \item tabellone di versionamento - ogni riga contiene:
        \begin{itemize}
            \item versione
            \item data - la data in cui il membro ha finito una certa modifica al documento
            \item descrizione - la spiegazione breve delle modifiche fatte al documento
            \item autore - chi ha effettuato le modifiche
            \item verificatore - chi ha verificato ed approvato le modifiche
        \end{itemize}
    \item indice
    \item contenuto vero e proprio del documento
\end{itemize}

Nel repository \textit{Git} di progetto è disponibile un template generico che serve come il punto di partenza nella redazione di documenti nuovi. Crea, posiziona e formatta gli elementi elencati sopra. L'idea è di avere una documentazione facilmente navigabile e con un aspetto professionale e standardizzato. 

\subsubsection{Produzione}
\subsubsection{Manutenzione}

\subsubsection{Documenti del progetto}
I documenti che il gruppo ha prodotto finora e ne sta manutenendo la maggioranza sono:
\begin{itemize}
    \item \textbf{Analisi dei Requisiti (AdR)} - raccoglie i \textit{casi d'uso} e i \textbf{requisiti} del progetto, stabiliti assieme all'agenzia \textbf{Proponente}
    \item \textbf{Piano di Progetto (PdP)} - gestisce i rischi possibili nel progetto, pianifica il progetto e ne segue l'andamento
    \item \textbf{Norme di Progetto (NdP)} - definisce il \textit{Way-of-Working} del gruppo
    \item \textbf{Glossario} - descrive i termini tecnici presenti nel resto della documentazione
    \item \textbf{Piano di Qualifica (PdQ)} - si occupa dei processi di verifica e validazione per assicurare la qualità del prodotto e dei processi
    \item \textbf{Lettere di presentazione} - una lettera per ogni degli eventi principali dell'insegnamento finora: candidatura al capitolato del progetto preferito e la revisione RTB. Queste lettere non vengono mantenute più dopo il loro invio al \textbf{Committente} via email. 
\end{itemize}
I loro scopi sono ben spiegati all'interno dei documenti stessi.

\subsubsection{Strumenti}
Gli strumenti che si utilizzano in questo processo sono:
\begin{itemize}
    \item \textit{Overleaf} - Un editor {\LaTeX} collaborativo basato su \textbf{\glo{cloud}{cloud}}. Utilizzato per la formattazione precisa dei file di documentazione e per il loro affinamento finale.

    \item \textit{TeXPage} - Un altro editor {\LaTeX} collaborativo basato su \textbf{\glo{cloud}{cloud}} con lo stesso uso come \textit{Overleaf}. Offre il vantaggio di più tempo di compilazione a disposizione, che rende possibile la compilazione di documenti {\LaTeX} grandi come ad esempio l'\textbf{Analisi dei Requisiti}.

    \item {[lo strumento per la verifica ortografica]}
    \item {[lo str. per l'indice di Gulpease, se usato]}
\end{itemize}
Gli altri strumenti trasversali che vengono utilizzati per supportare anche il processo di documentazione sono spiegati nella sezione 4.2 "Infrastruttura di Supporto". 
