...
Dal punto di vista dell'utente il sistema espone un'unica origine web. Il browser scarica il Frontend, un'applicazione React compilata in file statici, e da quel momento parla solo con il Backend: chiamate REST da un lato, un canale WebSocket dedicato agli aggiornamenti in tempo reale delle operazioni in corso dall'altro. Frontend e Backend restano comunque due unità distinte, e aggiornare l'una non richiede di ridistribuire l'altra: il loro unico punto di contatto è il contratto REST/WebSocket.
...
Lo store dedicato alle operazioni non viene scritto dall'interfaccia: lo popolano soltanto gli eventi che il Backend invia sul canale WebSocket già descritto in §3.1. C'è poi un meccanismo di riallineamento esplicito, perché quel canale non garantisce la consegna degli eventi emessi durante un'eventuale disconnessione: alla riconnessione il Frontend richiede di nuovo lo stato delle operazioni tramite API REST.
...
Un'analisi può fallire per ragioni molto diverse tra loro (un timeout del modello linguistico, un token GitHub non più valido, un limite di utilizzo superato), ma il sistema le riconduce tutte a un vocabolario chiuso di codici, condiviso tra Servizio Agenti e Backend nonostante i due linguaggi diversi. Un errore che nasce lato agente viene tradotto nel corrispondente codice del Backend mentre attraversa il confine tra i due servizi. Un codice che il Backend non riconosce non viene propagato così com'è: viene ricondotto a un valore generico.

Un normalizzatore centralizzato sul Backend traduce ogni errore in una risposta HTTP di forma identica (un codice, un messaggio, eventuali dettagli di validazione) prima che raggiunga il Frontend. Abbiamo riusato la stessa forma anche sul canale asincrono: un errore emerso a metà di una Task arriva al Frontend con lo stesso codice di una risposta REST, non con un messaggio libero costruito per l'occasione. \texttt{CREDENTIAL\_INVALID} ne è l'esempio più concreto: lo stesso codice vale sia quando GitHub rifiuta un token al salvataggio, sia quando quel token si rivela non funzionante a metà di un'operazione già avviata, e in entrambi i casi compare lo stesso avviso con reindirizzamento alla schermata delle credenziali.
...
Il meccanismo che rende possibile questo comportamento è il checkpoint di LangGraph (\texttt{AsyncMongoDBSaver}), che salva lo stato interno del grafo su MongoDB a ogni passaggio: è di LangGraph, non del codice scritto per questo progetto. Ciò che appartiene al progetto è tutto ciò che sta intorno a quel meccanismo. Delle tre sospensioni possibili di una Task Changelog (\S 3.5.3), solo due avvengono dentro un grafo già in esecuzione e usano davvero il checkpoint; la terza, quella per lo Sprint ID, avviene prima ancora che l'agente sia invocato e non ne ha bisogno. Il Backend rispetta la distinzione scegliendo, caso per caso, se invocare \texttt{/internal/agent/start} o \texttt{/internal/agent/resume}. A questo si aggiunge il protocollo di ripresa vero e proprio: un \texttt{threadId} per ogni esecuzione, e la ripresa tramite \texttt{POST /tasks/:id/input}, che azzera \texttt{pendingInput} e reinvoca l'agente con lo stesso \texttt{threadId}.

Il worker si libera subito dopo la sospensione, invece di restare bloccato, e chi salva o ripristina il checkpoint non ha bisogno di leggerne o interpretarne il contenuto: la stessa incapsulazione che il pattern richiede.
