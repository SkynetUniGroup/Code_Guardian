...
\begin{tabularx}{\textwidth}{|c|c|X|l|l|}
    ...
    %%%%%%
    0.10 & 2026/09/15 & Correzioni delle sezioni 1, 2 e 3 in seguito alla verifica & \makecell[l]{Sara Ristovic} & \makecell[l]{Nome Cognome} \\ \hline
    %%%%%%
    0.9 & 2026/09/15 & Correzione dei diagrammi e del testo nella sezione "Flusso dei dati" in seguito alla verifica & \makecell[l]{Dario Pirrone} & \makecell[l]{Nome Cognome} \\ \hline
    %%%%%%
    0.8 & 2026/09/14 & Scrittura della sezione "Copertura dei requisiti" & \makecell[l]{Sara Ristovic} & \makecell[l]{Dario Pirrone} \\ \hline
    %%%%%%
...
Lo store dedicato alle operazioni non riceve i propri dati soltanto dal canale WebSocket già descritto in §3.1; viene scritto anche da chiamate REST dirette — al caricamento iniziale della schermata e per riflettere localmente un annullamento richiesto dall'utente. Il canale WebSocket resta comunque la fonte primaria degli aggiornamenti di stato durante l'esecuzione di un'operazione. Poiché quel canale non garantisce la consegna degli eventi emessi durante un'eventuale disconnessione, alla riconnessione il Frontend richiede nuovamente lo stato delle operazioni tramite API REST, invece di fidarsi implicitamente del solo canale realtime.
...
Il percorso di scrittura, \texttt{GithubWriteService}, esiste come classe del tutto separata anziché come estensione della precedente, proprio per non indebolire quella garanzia: dipende da \texttt{GithubClientService}, ricevuto in dependency injection, solo per le proprie letture di supporto, ad esempio risolvere un branch o leggere il contenuto attuale di un file, ma apre branch, commit e Pull Request con un proprio client. Nessun agente vi ha accesso: a invocarlo, soltanto dopo che un agente ha già terminato la propria esecuzione, è \texttt{ProposalPublisherService} — descritto nel dettaglio nella sezione successiva insieme al resto del percorso che segue il risultato di un agente.
...
Prelevare un job non basta a garantirne l'esecuzione singola: la coda consegna ogni job almeno una volta, non esattamente una, e la stessa Task può quindi essere recapitata due volte a due worker concorrenti. \texttt{TaskProcessor} si protegge da questo con un meccanismo di claim: prima di invocare l'agente, marca la Task con un lease a tempo e un fencing token generato per l'occasione; un secondo worker che riceve lo stesso job trova il lease già preso e si ferma senza fare nulla, mentre il rilascio del lease a fine esecuzione è condizionato al possesso dello stesso token, cosicché un rilascio tardivo non possa mai cancellare la presa in carico di un worker successivo.

Il risultato di un agente completato non raggiunge il Frontend direttamente: attraversa prima \texttt{ProposalPublisherService}, che apre la Pull Request quando l'agente ha prodotto una Proposal — preservando comunque il diff nel Report anche se l'apertura fallisce — e solo dopo \texttt{ReportAssemblyService}, che costruisce il Report vero e proprio a partire da quel payload. È soltanto a valle di questi due passaggi che \texttt{TaskProcessor} invoca \texttt{EventsGateway} per notificare l'esito al Frontend.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|*{4}{>{\centering\arraybackslash}p{2.7cm}|}}
        ...
        Requisiti Funzionali & 86,5 & 2,7 & 1,8 & 9 \\ \hline
        Requisiti Qualitativi & 76,4 & 5,9 & 11,8 & 5,9 \\ \hline
        Requisiti di Vincolo & 100 & 0 & 0 & 0 \\ \hline
        Requisiti di Sicurezza & 80 & 20 & 0 & 0 \\ \hline
        Requisiti Desiderabili & 72,7 & 9,1 & 9,1 & 9,1 \\ \hline
        Requisiti Opzionali & 76,9 & 0 & 0 & 23,1 \\ \hline
