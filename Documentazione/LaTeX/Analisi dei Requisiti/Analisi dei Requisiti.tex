...
Il sistema \textit{Code Guardian} interagisce con diverse entità fisiche e logiche per portare a termine le proprie funzionalità. Al fine di descrivere in modo chiaro e non ambiguo i \textit{Casi d'Uso}, gli \textit{Attori} sono stati classificati in due categorie: \textbf{\textit{Attori} Primari}, ovvero gli utenti umani che interagiscono direttamente o indirettamente con il sistema per raggiungere un obiettivo, e \textbf{\textit{Attori} Secondari}, ovvero i sistemi, le piattaforme o i file esterni chiamati in causa dall'applicazione per completare le operazioni richieste.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            ...
            \item L'utente fornisce l'URL del \textit{repository} Git.
            \item Il sistema verifica che il \textit{repository} sia raggiungibile e accessibile.
            \item L'utente seleziona il riferimento di base da utilizzare, scegliendo tra branch, commit o pull request.
            \item Se l'utente seleziona un branch senza indicare un commit specifico, il sistema considera di default l'ultimo commit disponibile sul branch selezionato.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
    \begin{enumerate}
        \item L'utente indica uno o più file o directory da includere nell'analisi.
    ...
    \begin{itemize}
        ...
        \item Se supportato dalla configurazione del sistema, l'utente può selezionare l'intero repository come ambito di analisi.
    \end{itemize}
\end{itemize}
...
Il sistema adotta un approccio \textit{\glo{Human-in-the-loop}{human-in-the-loop}}, in modo che le proposte generate dagli agenti siano validate da un utente prima di essere applicate.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            ...
            \item Il sistema mostra all'utente un messaggio che descrive la causa dell'errore (es. \textit{repository} non accessibile, API key non valida, file di configurazione assente, formato del report non generabile).
...
\item
  \textbf{Trigger}: Il \textit{Security Auditor} richiede una scansione di sicurezza oppure l'\textbf{Orchestratore} avvia il processo secondo le modalità di esecuzione configurate.
\item
  \textbf{Precondizioni}:
  \begin{itemize}
  \item È stato selezionato un ambito di analisi (UC 2).
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            ...
            \item L’Agente 3 presenta al \textit{Security Auditor} il confronto tra codice originale e codice proposto.
        \end{enumerate}
...
\begin{xltabular}{\textwidth}{|c|c|D|F|}
    ...
    RF.1 & Obbligatorio & Il sistema deve consentire all'utente di selezionare il Repository Git da utilizzare come contesto di esecuzione degli agenti, specificando anche il riferimento di base desiderato tra branch, commit o pull request. & UC 1 \\ \hline
