...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            \hline
            \textbf{Versione} & \textit{0.9.0} \\ \hline
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|c|c|X|l|l|}
        ...
        0.9.1 & 2026/05/30 & Aggiunti 2 nuovi requisiti: RF.13 e RQ.3 & Dario Pirrone & Edoardo Granziero \\ \hline        
        0.9.0 & 2026/05/28 & Scrittura della sottosezione "Attori" (3.2) & Dario Pirrone & Edoardo Granziero \\ \hline
        0.8.2 & 2026/05/26 & Raffinati casi dell'agente DOX e adeguati gli UC al modello & Andrea Fistarol & Samuele Bertin \\ \hline
...
L'eventuale integrazione con \textit{\glo{GitHub Actions}{github-actions}} potrà essere utilizzata per automatizzare l'esecuzione degli agenti in seguito a eventi come push o \textit{\glo{Pull Request}{pull-request}}, qualora tale funzionalità venga inclusa nel perimetro del \textbf{\glo{MVP}{mvp}}.
\newpage
...
\begin{itemize}
    ...
    \item \textbf{Precondizioni}: Un agente ha prodotto una proposta di modifica. La proposta è disponibile sotto forma di \textit{Pull Request}, \textit{Patch} o suggerimento testuale [da decidere...?]
...
\newpage
\section{Agente DOX: Documentazione e \textit{README}}
...
Di seguito i \textit{Casi d'Uso} dell'Agente DOX.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'\textbf{Orchestratore} notifica l'Agente DOX della necessità di gestire il \textit{README}.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'\textbf{Orchestratore} notifica l'Agente DOX della necessità di intervento.
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'\textbf{Orchestratore} notifica l'Agente DOX della necessità di aggiornamento.
        ...
        \end{enumerate}
    \item \textbf{Inclusioni}: UC 4 - Validazione umana di una proposta dell’agente.
    \item \textbf{Estensioni}: UC 10 - Le modifiche al codice non richiedono aggiornamenti della documentazione.
\end{itemize}
...
\begin{itemize}
    \item \textbf{\textit{Attore} Primario}: Sviluppatore.
    \item \textbf{\textit{Attori} Secondari}: \textbf{Orchestratore}.
    \item \textbf{Trigger}: Richiesta esplicita da parte dello Sviluppatore o in automatico a seguito di un \textit{Commit} nella repo.
    \item \textbf{Precondizioni}:  
        \begin{itemize}
...
\begin{itemize}
    \item \textbf{\textit{Attore} Primario}: Sviluppatore.
    \item \textbf{\textit{Attori} Secondari}: \textbf{Orchestratore}.
    \item \textbf{Trigger}: Lo Sviluppatore effettua un \textit{Commit} con modifiche a uno o più file sorgente o fa richiesta esplicita.
    \item \textbf{Precondizioni}:  
        \begin{itemize}
            \item È stato selezionato un ambito di analisi (UC 2).
...
\subsubsection{UC 10: Le modifiche al codice non richiedono aggiornamenti della documentazione}
L'agente controlla la coerenza della documentazione \textit{API} dopo una modifica.
...
\subsubsection{UC 11: Esecuzione Scansione \textit{Vulnerabilità} (\textit{OWASP})}
...
\item
  ...
  \item È stato selezionato un ambito di analisi (UC 2).
  \item
    L'Agente OWASP è correttamente configurato con le \textit{API}
...
\subsubsection{UC 12: Verifica Conformità "\textit{Policy-as-code}" (CLAUDE.md)}
...
\subsubsection{UC 13: Generazione Suggerimenti di \textit{Remediation}}
...
\item
  ...
  \def\labelenumi{\arabic{enumi}.}
    ...
    \item 
    Se la vulnerabilità è localizzabile, l'Agente genera uno snippet di codice correttivo (UC 13.1).
    \item 
    Se la vulnerabilità non è localizzabile in modo sicuro, l'Agente genera una descrizione testuale della remediation (UC 13.2).
    \item
...
\subsubsection{UC 13.1: Generazione snippet di codice correttivo}
...
\begin{itemize}
    ...
    \item \textbf{Scenario Principale}:
        ...
        \end{enumerate}
    \item \textbf{Scenari Alternativi}: Se la modifica non è localizzabile in uno snippet, l’agente rimanda a UC 13.2.
\end{itemize}

\subsubsection{UC 13.2: Generazione descrizione testuale della \textit{Remediation}}
...
\subsubsection{UC 14: Recupero e Filtraggio \textit{User Stories}}
...
\subsubsection{UC 14.1: Rilevamento di task con metadati insufficienti}
\begin{itemize}
\item \textbf{\textit{Attore} Primario}: Sviluppatore / \textit{Project Manager}.
...
\subsubsection{UC 14.2: Gestione delle ambiguità e delle incoerenze}
\begin{itemize}
\item \textbf{\textit{Attore} Primario}: Sviluppatore / \textit{Project Manager}.
...
\subsubsection{UC 15: Generazione \textit{Changelog} Tecnico (Dev-to-Dev)}
\begin{itemize}
    \item \textbf{\textit{Attore} Primario}: Sviluppatore.
...
\subsubsection{UC 16: Generazione \textit{Changelog} di Business (Dev-to-Client)}
...
\begin{xltabular}{\textwidth}{|c|c|D|F|}
    ...
    RF.1 & Obbligatorio & Il sistema deve consentire all'utente di selezionare il Repository Git da utilizzare come contesto di esecuzione degli agenti, specificando anche il riferimento di base desiderato tra branch, \textit{Commit} o \textit{Pull Request}. & UC 1 \\ \hline
    ...
    \hline
    RF.12 & Obbligatorio & Il sistema deve generare o aggiornare la documentazione delle API del progetto. & UC 9, UC 10\\
    \hline
    RF.13 & Obbligatorio & Il sistema deve riconoscere quando le modifiche apportate al codice sorgente non alterano il comportamento, i componenti o gli endpoint analizzati, terminando l'esecuzione senza generare nuove proposte di aggiornamento documentale. & UC 10\\
    \hline
    ...
    \hline
    RF.21 & Obbligatorio & Il sistema deve recuperare e filtrare, da un sistema di Task Management esterno, le User Stories e i task completati relativi a uno specifico Sprint. & UC 14 \\
    \hline
    ...
    \hline
    RF.25 & Obbligatiorio & Il sistema deve rilevare ambiguità o incoerenze tra la documentazione della User Story e i dati tecnici associati, segnalandole all’utente per risoluzione manuale. & UC 14.2 \\
    \hline
    ...
    \hline
    RF.27 & Obbligatorio &  Il \textit{Changelog} tecnico deve includere i collegamenti ipertestuali e gli identificativi dei ticket di origine, quando tali informazioni sono disponibili. & UC 15 \\
    \hline
    RF.28 & Obbligatorio & Il sistema deve generare un \textit{Changelog} orientato al business, comprensibile a stakeholder non tecnici, a partire dai dati validati dello Sprint e/o dal \textit{Changelog} tecnico. & UC 16 \\
    \hline
...
\begin{xltabular}{\textwidth}{|c|c|D|F|}
    ...
    \hline
    RQ.3 & Desiderabile & Il sistema deve minimizzare i falsi positivi nella generazione della documentazione, evitando di proporre aggiornamenti per modifiche al codice puramente stilistiche o marginali. & UC 10 \\
    \hline
...
\begin{xltabular}{\textwidth}{|c|X|}
    ...
    \hyperref[uc:7]{UC 7} & RF.8 \\ \hline
    \hyperref[uc:8]{UC 8} & RF.9 \\ \hline
    \hyperref[uc:8.1]{UC 8.1} & RF.10 \\ \hline
    \hyperref[uc:8.2]{UC 8.2} & RF.11 \\ \hline
    \hyperref[uc:9]{UC 9} & RF.12 \\ \hline
    \hyperref[uc:10]{UC 10} & RF.12, RF.13, RQ.3 \\ \hline
...
\begin{xltabular}{\textwidth}{|c|X|}
    ...
    RF.10 & \hyperref[uc:8.1]{UC 8.1} \\ \hline
    RF.11 & \hyperref[uc:8.2]{UC 8.2} \\ \hline
    RF.12 & \hyperref[uc:9]{UC 9}, \hyperref[uc:10]{UC 10} \\ \hline
    RF.13 & \hyperref[uc:10]{UC 10} \\ \hline
    ...
    RQ.3 & \hyperref[uc:10]{UC 10} \\ \hline
