...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            \hline
            \textbf{Versione} & \textit{0.15.0} \\ \hline
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Email già registrata:} Al passo 5, se il sistema rileva che l'indirizzo email inserito è già associato a un account esistente, l'operazione viene interrotta. Il sistema mostra un messaggio di errore e suggerisce di procedere con il login (UC 21).
        \item \textbf{Dati non validi:} Al passo 5, se i dati inseriti non rispettano i criteri di validazione (password troppo debole o formato email errato), il sistema mostra un messaggio di errore specifico e chiede all'utente di correggere i campi evidenziati.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Scenari alternativi:}
    \begin{itemize}
        \item \textbf{Credenziali errate:} Al passo 4, se l'indirizzo email o la password non trovano corrispondenza nel sistema, l'operazione viene interrotta. Il sistema mostra un messaggio di errore generico (es. "Credenziali non valide") e invita l'utente a riprovare.
    \end{itemize}
\end{itemize}
...
\begin{xltabular}{\textwidth}{|c|c|D|F|}
    ...
    RF.37 & Obbligatorio & Il sistema deve consentire a un utente non autenticato di registrarsi creando un nuovo account e inserendo nome, cognome, email e password. & UC 20 \\ \hline
    RF.38 & Obbligatorio & Il sistema deve permettere all'utente, in fase di registrazione, di selezionare il proprio ruolo operativo (Sviluppatore, \textit{Security Auditor}, \textit{Project Manager}). & UC 20 \\ \hline
    RF.39 & Obbligatorio & Il sistema deve validare i dati inseriti (formato dell'email, robustezza della password) e mostrare un messaggio d'errore in caso di mancata conformità. & UC 20 \\ \hline
    RF.40 & Obbligatorio & Il sistema deve impedire la registrazione e mostrare un messaggio informativo se l'indirizzo email inserito è già associato a un account esistente. & UC 20 \\ \hline
    RF.41 & Obbligatorio & Il sistema deve consentire a un utente registrato di autenticarsi fornendo le proprie credenziali (email e password) e di conseguenza sbloccare i permessi e le funzionalità associate allo specifico ruolo dell'utente. & UC 21 \\ \hline
    RF.42 & Obbligatorio & IIl sistema deve negare l'accesso e mostrare un messaggio di errore generico nel caso in cui le credenziali inserite non siano valide. & UC 21 \\ \hline
