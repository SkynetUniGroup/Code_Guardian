...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            ...
            \textbf{Redattori} & \textit{Sara Ristovic, Dario Pirrone, Andrea Fistarol, Marco Barbiero} \\ \hline
            \textbf{Verifica} & \textit{Marco Barbiero, Samuele Bertin, Sara Ristovic, Edoardo Granziero, Andrea Fistarol} \\ \hline
            \textbf{Approvazione} & \textit{Edoardo Granziero, Andrea Fistarol, Marco Barbiero} \\ \hline
...
\begin{xltabular}{\textwidth}{|c|c|X|p{2.2cm}|p{2.2cm}|}
    ...
    2.0.0 & 2026/09/23 & Approvazione per la revisione PB & \makecell[l]{Marco \\Barbiero} & \makecell[l]{-} \\ \hline
    1.3.0 & 2026/09/22 & Verifica e correzione dell'ortografia e dell'impaginazione & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{-} \\ \hline
    1.2.0 & 2026/09/22 & Aggiunta di altri termini da Specifiche Tecniche & \makecell[l]{Samuele \\Bertin} & \makecell[l]{Sara \\Ristovic} \\ \hline
    1.1.0 & 2026/09/05 & Aggiunta di altri termini da NdP e PdP & \makecell[l]{Samuele \\Bertin} & \makecell[l]{Dario \\Pirrone} \\ \hline
    1.0.1 & 2026/08/21 & Rimossa la numerazione dall'indice, come suggerito dal Prof. Vardanega & \makecell[l]{Marco \\Barbiero} & \makecell[l]{Dario \\Pirrone} \\ \hline
    1.0.0 & 2026/08/13 & Approvazione del documento per revisione RTB & \makecell[l]{Andrea \\Fistarol} & -- \\ \hline
    0.4.0 & 2026/08/12 & Aggiunti nuovi termini & \makecell[l]{Andrea \\Fistarol} & \makecell[l]{Edoardo \\Granziero} \\ \hline
    0.3.0 & 2026/07/07 & Verifica dettagliata e correzione del documento intero & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Edoardo \\Granziero} \\ \hline
    0.2.0 & 2026/05/22 & Inseriti primi termini & \makecell[l]{Dario \\Pirrone} & \makecell[l]{Samuele \\Bertin} \\ \hline
    0.1.0 & 2026/04/03 & Stesura della sezione "Introduzione" & \makecell[l]{Sara \\Ristovic} & \makecell[l]{Marco \\Barbiero} \\ \hline
\end{xltabular}
}
...
\noindent\hypertarget{aaa-pattern}{\textbf{AAA Pattern (Arrange-Act-Assert)}} \\[0.1em]
Convenzione utilizzata per strutturare la scrittura dei \textit{Test di Unità}, suddividendone il contenuto in tre fasi distinte: \textit{Arrange} (preparazione dei dati e delle condizioni iniziali), \textit{Act} (esecuzione dell'operazione da testare) e \textit{Assert} (verifica del risultato ottenuto rispetto a quello atteso).
\vspace{0.5cm}
...
\noindent\hypertarget{asgi}{\textbf{ASGI}} \\[0.1em]
Specifica standard per server e applicazioni Python che supportano l'elaborazione di richieste in modo asincrono.
\vspace{0.5cm}
...
\noindent\hypertarget{backlog}{\textbf{Backlog}} \\[0.1em]
Elenco ordinato e dinamico di tutte le attività, funzionalità o correzioni ancora da svolgere all’interno di un progetto. Nel contesto del progetto, raccoglie le \textit{issue} non ancora prese in carico o rimaste in sospeso al termine di uno \textit{sprint}, in attesa di essere pianificate in quelli successivi.
\vspace{0.5cm}
...
\noindent\hypertarget{build}{\textbf{Build}} \\[0.1em]
Processo automatizzato di compilazione, assemblaggio e verifica sintattica (tramite linter) del codice sorgente di un progetto, finalizzato a produrre una versione eseguibile o distribuibile del software. Nel progetto viene eseguito nell’ambito degli strumenti di \textit{Continuous Integration} (CI) per validare che le modifiche apportate al codice non introducano errori.
\vspace{0.5cm}
...
Soggetto che richiede e finanzia lo sviluppo del sistema, identificato nel progetto con i docenti universitari (Prof. Tullio Vardanega, Prof. Riccardo Cardin).
\vspace{0.5cm}
...
Bilancio redatto a fine \textit{sprint} che rendiconta in modo trasparente le ore, le risorse e i costi effettivamente spesi confrontandoli con le stime previste.
\vspace{0.5cm}
...
\noindent\hypertarget{conventional-commits}{\textbf{Conventional Commits}} \\[0.1em]
Convenzione standardizzata per la scrittura dei messaggi di \textit{Commit}, che ne struttura il contenuto nella forma \texttt{tipo(ambito): descrizione} (ad esempio \texttt{feat}, \texttt{fix}, \texttt{docs}). Facilita la leggibilità della cronologia del \textit{repository} e la generazione automatica dei \textit{Changelog}. Adottata dal gruppo come standard per la scrittura di tutti i commit del progetto.
\vspace{0.5cm}
...
\noindent\hypertarget{design-pattern}{\textbf{Design-Pattern}} \\[0.1em]
Soluzione progettuale generale, riutilizzabile e collaudata, adottata per risolvere un problema ricorrente nella progettazione del software. Non rappresenta codice pronto all’uso, ma uno schema concettuale di riferimento che guida le scelte architetturali del Progettista durante il system design.
\vspace{0.5cm}
...
\noindent\hypertarget{docker-compose}{\textbf{Docker Compose}} \\[0.1em]
Strumento che permette di definire e gestire applicazioni composte da più container tramite un file di configurazione dichiarativo. Nel progetto viene utilizzato durante i \textit{Test di Integrazione} per avviare rapidamente servizi esterni (come un'istanza di MongoDB) in un ambiente isolato e riproducibile.
\vspace{0.5cm}

\noindent\hypertarget{dto}{\textbf{DTO}} \\[0.1em]
Oggetto utilizzato per trasferire dati tra sottosistemi o attraverso un confine di rete, tipicamente validato in ingresso e privo di logica di business propria.
\vspace{0.5cm}
...
\noindent\hypertarget{earned-value-management}{\textbf{Earned Value Management (EVM)}} \\[0.1em]
...
\noindent\hypertarget{elementi-configurazione}{\textbf{Elementi di Configurazione}} \\[0.1em]
...
\noindent\hypertarget{fixture}{\textbf{Fixture}} \\[0.1em]
Insieme di dati o configurazioni predefinite utilizzate per preparare un ambiente di test controllato e riproducibile, garantendo che ogni test parta da uno stato noto e consistente.
\vspace{0.5cm}
...
\noindent\hypertarget{git-flow}{\textbf{Git Flow}} \\[0.1em]
Modello di \textit{Branching} per \textit{Git} che definisce una struttura standardizzata di rami (tipicamente \texttt{main}, \texttt{develop}, \texttt{feature}, \texttt{release} e \texttt{hotfix}) per organizzare lo sviluppo parallelo e il rilascio del software.
\vspace{0.5cm}
...
\noindent\hypertarget{hmac}{\textbf{HMAC}} \\[0.1em]
Meccanismo crittografico che genera un codice di autenticazione a partire da un messaggio e una chiave condivisa, usato per verificare che una richiesta provenga da un chiamante autorizzato e non sia stata alterata in transito.
\vspace{0.5cm}
...
\noindent\hypertarget{iam}{\textbf{IAM}} \\[0.1em]
Servizio o insieme di funzionalità che gestisce identità, permessi e credenziali di accesso alle risorse di un'infrastruttura cloud.
\vspace{0.5cm}
...
\noindent\hypertarget{jwt}{\textbf{JWT}} \\[0.1em]
Standard aperto per la rappresentazione di informazioni (claim) in forma compatta e verificabile tramite firma digitale, utilizzato per autenticare sessioni utente senza che il server debba mantenerne lo stato.
\vspace{0.1cm}
...
\noindent\hypertarget{kms}{\textbf{KMS}} \\[0.1em]
Servizio gestito per la creazione, la conservazione e la rotazione delle chiavi crittografiche usate per cifrare dati e segreti.
\vspace{0.1cm}
...
\noindent\hypertarget{linter}{\textbf{Linter}} \\[0.1em]
Strumento di \textit{Analisi statica} che esamina il codice sorgente per individuare errori di sintassi, violazioni delle convenzioni di stile o pattern potenzialmente problematici, senza eseguire il codice. Nel progetto viene impiegato tramite Biome per la componente TypeScript e Pylint per la componente Python.
\vspace{0.5cm}
...
\noindent\hypertarget{lockfile}{\textbf{Lockfile}} \\[0.1em]
File generato automaticamente da un package manager (nel progetto, pnpm) che registra le versioni esatte di tutte le dipendenze installate, garantendo installazioni identiche e riproducibili su ambienti diversi. Nel progetto corrisponde al file \texttt{pnpm-lock.yaml}, che non deve mai essere modificato manualmente.
\vspace{0.1cm}
\newpage
...
\noindent\hypertarget{maintainability-index}{\textbf{Maintainability Index (Indice di Manutenibilità)}} \\[0.1em]
...
\noindent\hypertarget{nat-gateway}{\textbf{NAT Gateway}} \\[0.1em]
Componente di rete che permette a risorse prive di indirizzo pubblico, situate in una subnet privata, di raggiungere Internet in uscita senza essere direttamente raggiungibili dall'esterno.
\vspace{0.5cm}
...
\noindent\hypertarget{odm}{\textbf{ODM}} \\[0.1em]
Libreria che mappa oggetti del linguaggio di programmazione su documenti di un database non relazionale, semplificandone la lettura e la scrittura senza dover comporre query native.
\vspace{0.5cm}

\noindent\hypertarget{oidc}{\textbf{OIDC}} \\[0.1em]
Protocollo di autenticazione basato su OAuth 2.0 che consente a un sistema di verificare l'identità di un chiamante tramite un provider di identità esterno, senza scambiare credenziali statiche di lunga durata.
\vspace{0.5cm}
...
\noindent\hypertarget{pytest}{\textbf{Pytest}} \\[0.1em]
Framework di test per il linguaggio Python, diffuso per la sua sintassi concisa e l'ampio ecosistema di plugin. Nel progetto viene utilizzato per l'esecuzione dei \textit{Test di Unità} e dei \textit{Test di Integrazione} della componente Python, con configurazione definita nel file \texttt{pytest.ini}.
\vspace{0.5cm}
...
Metodologia di lavoro ibrida che unisce il framework di Scrum (come gli \textit{sprint} e le riunioni cicliche) con la flessibilità e la gestione visiva del flusso di lavoro tipiche del Kanban. È pensata per migliorare il flusso di lavoro riducendo gli overhead gestionali.
\vspace{0.5cm}
...
\noindent\hypertarget{tls}{\textbf{TLS}} \\[0.1em]
Protocollo crittografico che garantisce riservatezza e integrità delle comunicazioni di rete, alla base dello scambio dati cifrato via HTTPS.
\vspace{0.5cm}
...
\noindent\hypertarget{trigger}{\textbf{Trigger}} \\[0.1em]
Evento che avvia automaticamente l'esecuzione di un processo. Nel contesto degli \textit{Use Case}, identifica l'evento che dà inizio allo scenario descritto; nel contesto della \textit{Continuous Integration}, identifica l'evento (ad esempio una \textit{Pull Request}) che avvia automaticamente una pipeline di \textit{GitHub Actions}.
\vspace{0.5cm}

\noindent\hypertarget{typescript}{\textbf{TypeScript}} \\[0.1em]
Linguaggio di programmazione open-source sviluppato da Microsoft come superset di JavaScript, che ne estende la sintassi introducendo un sistema di tipizzazione statica opzionale. Il codice TypeScript viene convertito (transpiled) in JavaScript standard prima dell'esecuzione. Nel progetto è il linguaggio adottato per la componente Backend (tramite NestJS) e per la relativa suite di test.
\vspace{0.5cm}
...
\noindent\hypertarget{vite}{\textbf{Vite}} \\[0.1em]
Strumento di build e ambiente di sviluppo per applicazioni JavaScript/TypeScript. Fornisce un server di sviluppo con ricaricamento rapido durante lo sviluppo e orchestra il processo di build per la produzione. Nel progetto è configurato tramite il file \texttt{vite.config.ts} e si appoggia a Oxc per il transpiling e a Rolldown per il bundling.
\vspace{0.5cm}

\noindent\hypertarget{vitest}{\textbf{Vitest}} \\[0.1em]
Framework di test per applicativi JavaScript/TypeScript, integrato nativamente con l'ecosistema \textit{Vite}. Nel progetto viene utilizzato per i \textit{Test di Unità} e i \textit{Test di Integrazione} della componente TypeScript, con copertura del codice misurata tramite il motore V8.
\vspace{0.5cm}

\noindent\hypertarget{vpc}{\textbf{VPC}} \\[0.1em]
Rete virtuale isolata all'interno di un provider cloud, che consente di definire in modo controllato subnet, instradamento e regole di accesso per le risorse che vi risiedono.
\vspace{0.5cm}
...
\noindent\hypertarget{websocket}{\textbf{WebSocket}} \\[0.1em]
Protocollo di comunicazione che stabilisce un canale bidirezionale e persistente tra client e server, utilizzato per inviare aggiornamenti in tempo reale senza che il client debba interrogare ripetutamente il server.
\vspace{0.5cm}
