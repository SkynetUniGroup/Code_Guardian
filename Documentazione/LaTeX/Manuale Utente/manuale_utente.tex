...
\fancyhead[R]{\includegraphics[height=11pt]{figures/logo_skynet_dark.png}}
...
\hline
\textbf{Verifica} & \textit{Nome Cognome} \\
\hline
\textbf{Approvazione} & \textit{Dario Pirrone} \\
\hline
\end{tabular}
...
\hline
0.1 & 2026/09/03 & Prima stesura del Manuale Utente: Introduzione, Installazione e Avvio, Guida all'interfaccia, Funzionalità AI, Risoluzione dei problemi ed errori, Glossario. & Samuele Bertin & Nome Cognome \\
\hline
\end{tabularx}
...
\begin{itemize}
  ...
  \item \textbf{File e cartelle specifici} --- tramite un albero di selezione a tre stati
        (non selezionato, selezionato, parzialmente selezionato): selezionare una cartella
        ne marca automaticamente tutto il contenuto, in modo ricorsivo.
\end{itemize}

Un contatore in tempo reale mostra il numero di file coinvolti nella selezione. Confermando
con \textbf{Conferma contesto}, il contesto viene salvato e reso disponibile a tutte le
...
Tabella~\ref{tab:ruoli}), ciascuna con nome, breve descrizione e casella di selezione. È
...
\begin{longtable}{p{4.3cm}p{5.0cm}p{4.2cm}}
\toprule
...
\begin{description}[leftmargin=3.4cm, style=nextline]
  ...
  \item[Repository] Il progetto software ospitato su GitHub su cui vengono eseguite le
    analisi.
