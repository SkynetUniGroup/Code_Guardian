...
\subsubsection{DOCS\_INLINE (RF.83, RF.84)}
...
Instradamento: \texttt{AgentRegistry} → Agente Security. Il backend invoca l'agente e persiste il Report ricevuto (elenco di \texttt{FindingBlock}) senza ulteriori passi. Ogni \texttt{FindingBlock} porta \texttt{category} (OWASP), \texttt{severity}, \texttt{filePath}, \texttt{lineStart}/\texttt{lineEnd?} e \texttt{remediation} (discriminante \texttt{SNIPPET | TEXT}). Il backend riceve e persiste questi valori così come arrivano.
