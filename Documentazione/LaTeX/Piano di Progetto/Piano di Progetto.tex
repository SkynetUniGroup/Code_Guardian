...
\begin{itemize}
     ...
     \item L'analisi del consuntivo orario dello \textit{Sprint} 5 ha evidenziato un superamento delle ore effettive rispetto a quelle inizialmente pianificate.\\
     Per risolvere questo problema, il monte ore del ruolo di \textit{Progettista} è aumentato per coprire lo studio dell'architettura dell'orchestratore e l'avvio del documento di specifica del \textit{PoC}. Di contro, le ore di verifica sono state riallocate per garantire il controllo qualità sui documenti parzialmente riscritti.\\
     Il sovraccarico orario è stato assorbito sfruttando la flessibilità del monte ore totale del periodo di baseline, senza intaccare il budget finale ma richiedendo un efficientamento per lo \textit{Sprint} successivo.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Pianificazione oraria predittiva basata sullo storico:} Al fine di rientrare nel budget orario ed evitare ulteriori scostamenti tra ore previste e ore fatte, per lo \textit{Sprint} 6 verrà adottata una stima dei task più conservativa, tenendo conto del reale coefficiente di produttività emerso nello \textit{Sprint} 5.
