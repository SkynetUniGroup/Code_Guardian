...
\begin{titlepage}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            \hline
            \textbf{Versione} & \textit{0.13.1} \\ \hline
    
            \textbf{Uso} & \textit{Esterno} \\ \hline
            \textbf{Destinatari} & \textit{Prof. Tullio Vardanega, Prof. Riccardo Cardin} \\ \hline
            \textbf{Redattori} & \textit{Dario Pirrone, Sara Ristovic, Edoardo Granziero, Marco Barbiero, Samuele Bertin} \\ \hline
...
\begin{xltabular}{\textwidth}{|c|c|X|p{3.5cm}|p{3.5cm}|}
    ...
    \endhead
        ...
        0.14.0 & 2026/08/07 & Correzione e adeguamento della sezione \textit{3 Pianificazione} & Sara Ristovic &  \\ \hline 
        0.13.1 & 2026/08/05 & Implementazione delle correzioni della verifica dettagliata & Sara Ristovic & Samuele Bertin \\ \hline 
        0.13.0 & 2026/08/05 & Scrittura \textit{consuntivo} \textit{sprint} 10 e \textit{preventivo} \textit{sprint} 11 & Dario Pirrone &  \\ \hline
...
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2025/Dispense/T02.pdf}{Slide T02 del corso Ingegneria del Software (Processi di Ciclo di Vita)}
\item 
...
\item
  \href{https://www.math.unipd.it/~tullio/IS-1/2025/Dispense/T04.pdf}{Slide T04 del corso Ingegneria del Software (Gestione di Progetto)}
...
\\ \hline
        Precauzioni & Adottare inizialmente una pianificazione pessimistica, definire attività, compiti ed obiettivi molto chiari e con un monitoraggio costante dell'andamento del progetto tramite \textit{Consuntivi} di periodo.
\\ \hline
...
\\ \hline
        Probabilità di occorrenza & Alta.
\\ \hline
        Grado di pericolosità & Media-Bassa.
\\ \hline
        Precauzioni &
...
Include anche la realizzazione di un \textbf{Proof of Concept} (\textbf{PoC}) per validare la fattibilità dell'idea del prodotto e le scelte teconlogiche.
...
L'obiettivo è arrivare al rilascio di un \textit{MVP} funzionante validato, pronto per il collaudo formale con il \textbf{\glo{Committente}{committente}} e per l'approvazione da parte del \textbf{\glo{Proponente}{proponente} VarGroup}.
\end{itemize}
...
La fase di lavoro dedicata al raggiungimento della \textit{milestone} \textbf{RTB} si estende dall'inizio di aprile fino a metà agosto ed è focalizzata sul consolidamento delle basi teoriche, metodologiche e tecnologiche del progetto. Inizialmente era progettata di durare fino a metà giugno.
Seguono le sue sotto-\textit{milestone}, ognuna con la previsione iniziale di completamento, che purtpoppo non si sono verificate.
...
Fase pratica di realizzazione di alcuni requisiti precedentemente stabili e dell'implementazione delle scelte teconolgiche principali.
\begin{itemize}
...
\begin{itemize}
    \item Completamento di tutta la documentazione (rilascio versione v1.0.0) e validazione generale.
\item Preparazione per i colloqui di revisione \textbf{RTB}.
\end{itemize}

\subsubsection{Consuntivo della \textit{milestone} \textbf{RTB}}
Rispetto alle preveisioni, la fase di lavoro in preparazione di questa \textit{milestone} si è prolungata di due mesi. Ciò è dovuto a più ragioni:
\begin{itemize}
    \item Definizione non completamente adatta di alcune sotto-\textit{milestone};
    \item Implementazione e rispetto insufficiente delle sotto-\textit{milestone};
    \item Periodo molto lungo di adozione delle metodologie di lavoro definite nel \textbf{Norme di Progetto};
    \item Lavoro svolto che dopo è risultato di qualità insufficiente ed è dovuto essere ripetuto.
\end{itemize}

Nella prossima fase di lavoro, il gruppo si impegnerà a prevenire che gli stessi rischi si verificassero. È obbligatorio alzare la propria efficienza nel periodo restante, per riuscire a concludere il progetto entro la scadenza finale del 25 settembre.


\subsection{Product Baseline (PB)}
La fase di sviluppo dedicata al raggiungimento della \textit{milestone} \textbf{PB} prende il via a metà agosto per concludersi all'utlimo terzo di settembre. L'obiettivo è concretizzare il lavoro di analisi nello sviluppo del prodotto finale. È suddivisa nelle seguenti sotto-\textit{milestone}:
...
\item \textbf{Piano di Qualifica}: stesura degli obliettivi di qualità di processo;
...
\paragraph{Analisi dei rischi occorsi}
\begin{itemize}
     \item Durante lo \textit{sprint} si è concretizzato il rischio di un disallineamento informativo con il \textbf{Proponente} riguardo l'architettura di partenza. L'assenza di un \textbf{orchestratore} agentico preesistente sviluppato da \textbf{VarGroup}, contrariamente a quanto dichiarato precedentemente dall'azienda, richiederà uno sforzo aggiuntivo.\\
...
\begin{itemize}
    \item  Meodifiche all'\textbf{Analisi dei Requisiti} in preparzione del ricevimento con il Prof. Cardin
...
È risultato un consumo orario n linea con quanto preventivato inizialmente.
...
\paragraph{\textit{Retrospettiva}}Questo sprint è stato caratterizzato da una produttività ridotta a causa della concomitanza con la sessione esami, questa dinamica era stata ampliamente preventivata e il team è riuscito a fare i progressi deisiderati.
\paragraph{Cosa ha funzionato:}
\begin{itemize}
...
per l'\textbf{Amministratore}, mentre per i \textbf{Progettisti} e \textbf{Verificatori} il consumo complessivo risulta ugale ma con una ripartizione oraria per membro diversa da quanto preventivato.
...
\paragraph{\textit{Consuntivo}}
È risultato un consumo orario complessivo lievemente inferiore a quanto preventivato, in genrale con meno ore spese nel ruolo di \textbf{Verificatore} e lievemente di più per il ruolo di \textbf{Programmatore}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.9\textwidth}{|X|c|c|c|c|c|c|c|}
        ...
        \textbf{Ore totali per ruolo} & \textbf{} & \textbf{} &
        \textbf{0} & \textbf{0} & \textbf{0} & \textbf{} & \textbf{}\\ \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.9\textwidth}{|X|c|c|c|c|c|c|c|}
        ...
        \textbf{Costo orario} & \EUR{30} & \EUR{20} & \EUR{25} & \EUR{25} & \EUR{15}
        & \EUR{15} & / \\ \hline
        \textbf{Costo totale} & \EUR{0} & \EUR{0} & \EUR{0} & \EUR{0} & \EUR{0} & \EUR{0} & \textbf{\EUR{0}} \\ \hline
    \end{tabularx}
