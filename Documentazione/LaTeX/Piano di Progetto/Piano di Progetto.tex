...
\paragraph{Analisi dei rischi occorsi}
\begin{itemize}
     \item Durante lo sprint si è concretizzato il rischio di un disallineamento informativo con il \textbf{Proponente} riguardo l'architettura di partenza. L'assenza di un orchestratore software proprietario preesistente, contrariamente alle ipotesi preliminari, richiederà uno sforzo aggiuntivo.\\
     L'imprevisto ha causato un incremento del carico di lavoro e un rallentamento sulla tabella di marcia originaria.\\
     Il team ha ridefinito tempestivamente i vincoli tecnologici ed ha avviato lo studio e la progettazione interna del motore di orchestrazione.

     \item L'analisi del consuntivo orario dello Sprint 5 ha evidenziato un superamento delle ore effettive rispetto a quelle inizialmente pianificate.\\
     Per risolvere questo problema ,il monte ore del ruolo di Progettista è aumentato per coprire lo studio dell'architettura dell'orchestratore e l'avvio del documento di specifica del PoC. Di contro, le ore di verifica sono state riallocate per garantire il controllo qualità sui documenti parzialmente riscritti.\\
     Il sovraccarico orario è stato assorbito sfruttando la flessibilità del monte ore totale del periodo di baseline, senza intaccare il budget finale ma richiedendo un efficientamento per lo sprint successivo.
\end{itemize}

\paragraph{Retrospettiva: cosa ha funzionato}
\begin{itemize}
    \item \textbf{Avvio tempestivo e strutturato del PoC:} Nonostante l'imprevisto tecnico riscontrato con l'azienda, l'assegnazione mirata delle ore al ruolo di \textit{Progettista (PG)} ha permesso di definire con successo lo scheletro architetturale del \textbf{Proof of Concept} e di completare una parte significativa del suo sviluppo funzionale.
    \item \textbf{Raffinamento proattivo dei requisiti (AdR):} Il processo di verifica interna ha funzionato con efficacia: il team ha individuato le aree critiche dell'Analisi dei Requisiti (AdR) e ha avviato una sessione di correzione e irrobustimento del tracciamento, preparandosi in modo ottimale al colloquio di revisione con il Prof. Cardin.
\end{itemize}

\paragraph{Migliorie per lo Sprint 6}
\begin{itemize}
    \item \textbf{Pianificazione oraria predittiva basata sullo storico:} Al fine di rientrare nel budget orario ed evitare ulteriori scostamenti tra ore previste e ore fatte, per lo Sprint 6 verrà adottata una stima dei task più conservativa, tenendo conto del reale coefficiente di produttività emerso nello Sprint 5.
    \item \textbf{Parallelizzazione tra Sviluppo del PoC e Correzione Documentale:} Sfruttando lo scheletro del PoC ormai consolidato, si prevede di separare nettamente i flussi di lavoro nello Sprint 6.
\end{itemize}
...
\begin{itemize}
    ...
    \item verifica e modifica della documentazione per l'RTB.
\end{itemize}
