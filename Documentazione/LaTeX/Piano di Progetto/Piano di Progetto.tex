...
La pianificazione delle attività è stata definita per garantire uno sviluppo strutturato e incrementale del progetto, il cui obiettivo principale è la realizzazione di tre agenti (agente README, agente OWASP e agente changelog). Il periodo di lavoro ufficiale si estende dal 10 marzo fino alla data di scadenza finale prevista per il 10 settembre. Per gestire al meglio le risorse e rispettare le scadenze, il ciclo di vita del progetto è stato suddiviso in periodi di lavoro ben definiti.
...
\begin{itemize}
    ...
    \item \textbf{PB/MVP (Product Baseline / Minimum Viable Product):} In questa fase il focus si sposta sulla progettazione architetturale di dettaglio e sullo sviluppo effettivo del prodotto. L'obiettivo è arrivare al rilascio di un MVP funzionante, integrato e collaudato, pronto per l'approvazione formale da parte del committente (VarGroup).
\end{itemize}
...
\begin{itemize}
    \item Definizione del \emph{Way of Working} del team.
    \item Redazione del documento Norme di Progetto (NdP).
\end{itemize}
...
\begin{itemize}
    \item Analisi dei Requisiti (AdR): individuazione e completamento di tutti gli \emph{use case} e dei requisiti di sistema.
    \item Piano di Qualifica (PdQ): definizione delle metriche di qualità.
...
\begin{itemize}
    \item Pianificazione e realizzazione del Proof of Concept (PoC).
...
\begin{itemize}
    ...
    \item Preparazione per il colloquio di revisione RTB.
\end{itemize}

\subsection{Product Baseline e MVP (PB/MVP)}
...
\begin{itemize}
    ...
    \item Integrazione degli agenti con l'orchestratore esterno.
\end{itemize}
...
\begin{itemize}
    \item Esecuzione dei test di unità e dei test di integrazione.
...
\begin{itemize}
    \item Dimostrazione pratica dell'MVP a VarGroup e approvazione formale.
...
\begin{table}[H]
    \centerline{
        ...
        \begin{tabularx}{1.1\textwidth}{|m{5em}|p{11em}|p{7.5em}|X|}
            ...
            \multirow{4}{*}[-2cm]{\centering \textbf{RTB}} & Fondamenta e normativa & Inizio Maggio & - Definito Way of Working \newline
            ...
            \hline \hline
            \multirow{5}{*}[-2cm]{\centering \textbf{PB/MVP}} & Progettazione architetturale & Fine Luglio & - Redatte Specifiche Tecniche (ST) \\ 
            \cline{2-4}
...
\subsubsection{Sprint 2: 14/04/2026 - 28/04/2026}
...
E' risultato un consumo orario correttamente preventivato inizialmente per tutti i ruoli. 
...
\begin{table}[H]
    ...
    \end{tabularx}
    \caption{Consuntivo orario - Sprint 2}
\end{table}
...
\begin{table}[H]
    \centerline{
        ...
        \begin{tabularx}{1.09\textwidth}{|X|c|c|c|c|c|c|c|}
            \hline
            \textbf{Sprint 2} & \textbf{RE} & \textbf{AM} & \textbf{AN} & \textbf{PG} & \textbf{PR} & \textbf{VE} & \textbf{Totale} \\ \hline
            \textbf{Costo orario} & \EUR{30} & \EUR{20} & \EUR{25} & \EUR{25} & \EUR{15} & \EUR{15} & / \\ \hline
            \textbf{Costo totale} & \EUR{120} & \EUR{80} & \EUR{175} & \EUR{0} & \EUR{0} & \EUR{60} & \textbf{\EUR{435}} \\ \hline
            \textbf{Saldo a fine sprint} & \textbf{\EUR{1.470}} & \textbf{\EUR{820}} & \textbf{\EUR{1.225}} 
            & \textbf{\EUR{2.700}} & \textbf{\EUR{2.070}} & \textbf{\EUR{2.175}} & \textbf{\EUR{10.460}}\\ \hline
        \end{tabularx}
    }
    \caption{Consuntivo economico - Sprint 2}
\end{table}

\paragraph{Analisi dei rischi occorsi}
Durante lo svolgimento del secondo Sprint, l'applicazione delle misure di mitigazione per il rischio \textbf{RO1 (Genericità dei task)} ha prodotto esiti positivi, tuttavia l'aumento della complessità gestionale ha fatto emergere una nuova criticità:

\begin{itemize}
    \item \textbf{Difficoltà nel coordinamento dei ruoli (RO2):} Con l'introduzione di una scomposizione più fine delle attività in issue atomiche, è emersa una difficoltà iniziale nel coordinare il lavoro sincrono tra i diversi ruoli.
    \item \textbf{Mitigazione:} Per risolvere tempestivamente l'intoppo, il gruppo ha deciso di avere una comunicazione più frequente tramite Whatsapp e discord. Questa misura ha permesso di ridistribuire il carico di lavoro in tempo reale, garantendo agli Analisti le linee guida necessarie per procedere senza ulteriori blocchi.
    \item \textbf{Issue non chiuse:} Non siamo riusciti a chiudere tutte le issue aperte a cause delle difficoltà che abbiamo incontrato nella gestione dei ruoli e delle proprie ore(come si vede nel grafico burndown).
\end{itemize}

\paragraph{Retrospettiva}
Il bilancio dello Sprint 2 mostra un netto miglioramento qualitativo nel metodo di lavoro e una gestione più consapevole delle risorse economiche e temporali rispetto al periodo precedente.

\paragraph{Cosa ha funzionato:}
\begin{itemize}
    \item \textbf{Efficacia della granularità:} La scomposizione delle macro-attività in issue atomiche ha permesso un monitoraggio molto più preciso; lo stato di avanzamento dell'Analisi dei Requisiti è risultato chiaro a tutti i membri del gruppo in ogni momento.
    \item \textbf{Monitoraggio Data-driven:} L'adozione dei \textit{Burndown Chart} è stata fondamentale per identificare precocemente un rallentamento a metà Sprint, consentendo al Responsabile di intervenire proattivamente prima che la scadenza diventasse critica.
\end{itemize}

\paragraph{Migliorie per lo Sprint 3:}
\begin{itemize}
    ...
    \item \textbf{Standardizzazione dei documenti:} Per ottimizzare i tempi di formattazione LaTeX, che sono risultati superiori al previsto, il team ha deciso di sviluppare dei template pre-impostati per ogni tipologia di documento, velocizzando così la futura stesura tecnica.
\end{itemize}

\subsubsection{Sprint 3: 28/04/2026 - 12/05/2026}

\paragraph{Obiettivi e attività}

Le attività previste per il terzo \emph{Sprint} sono le seguenti:
...
\item
  \emph{Analisi dei requisiti}: continuare gli \emph{use case} dei 3
  agenti da sviluppare;
\item
  \emph{Norme di progetto}: continuare i processi primari e processi
  organizzativi;
\item
  \emph{Piano di progetto}: stesura Pianificazione, scrivere consuntivo \emph{Sprint} 2 e preventivo \emph{Sprint} 3.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.9\textwidth}{|X|c|c|c|c|c|c|c|}
        \hline
        \textbf{Sprint 3} & \textbf{RE} & \textbf{AM} & \textbf{AN} &
        \textbf{PG} & \textbf{PR} & \textbf{VE} & \textbf{Totale} \\ \hline
    ...
    \end{tabularx}
    \caption{Preventivo orario - Sprint 3}
\end{table}
...
\begin{table}[H]
    ...
    \end{tabularx}
    \caption{Preventivo economico - Sprint 3}
\end{table}
...
\begin{table}[H]
    \centerline{
        ...
        \begin{tabularx}{1.09\textwidth}{|X|c|c|c|c|c|c|c|}
            ...
            \textbf{Costo totale} & \EUR{150} & \EUR{180} & \EUR{275} & \EUR{0} & \EUR{0} & 
            \EUR{45} & \textbf{\EUR{650}} \\ \hline
            \textbf{Saldo a fine sprint} & \textbf{\EUR{1.320}} & \textbf{\EUR{640}} & \textbf{\EUR{950}} 
...
Durante lo svolgimento del terzo \emph{Sprint} è riemersa una criticità legata alla pianificazione (\textbf{RO1 - Scarsa Pianificazione}). La divisione delle \emph{issue} non si è rivelata sufficientemente granulare: avere \emph{task} troppo ampi non ci ha permesso di tracciare i progressi nel migliore dei modi. Questo ha reso la lettura del \emph{Burndown Chart} poco indicativa dello stato reale dei lavori e, soprattutto, ha generato un collo di bottiglia, impedendoci di chiudere definitivamente diverse attività prima della fine dello \emph{Sprint}.

\paragraph{Retrospettiva}

Il terzo \emph{Sprint} ha consolidato l'infrastruttura documentale, ma ha evidenziato evidenti limiti operativi nella gestione e chiusura dei \emph{task}.

\textbf{Cosa ha funzionato:}
\begin{itemize}
    \item \textbf{Standardizzazione in LaTeX:} L'adozione dei nuovi template in LaTeX ha reso il lavoro documentale molto più ordinato. L'omogeneità stilistica e la struttura preimpostata hanno facilitato notevolmente la stesura, ripagando l'investimento di tempo fatto nello \emph{Sprint} precedente e rendendo i documenti molto più manutenibili.
\end{itemize}

\textbf{Cosa migliorare per lo Sprint 4:}
\begin{itemize}
    \item \textbf{Scomposizione atomica e chiusura delle \emph{issue}:} A causa della scarsa granularità, molte attività previste non sono arrivate allo stato di "Done" e sono slittate allo \emph{Sprint} successivo. Per lo \emph{Sprint} 4, il team si impegnerà a spezzare i \emph{task} in unità di lavoro molto più piccole e indipendenti, per garantirne la chiusura tempestiva, evitare lo \emph{spillover} tra \emph{Sprint} e permettere un tracciamento continuo e reale dei progressi.
\end{itemize}

\subsubsection{Sprint 4: 12/05/2026 - [Data di fine]}

\paragraph{Obiettivi e attività}

Le attività previste per il quarto \emph{Sprint} sono le seguenti:

\begin{itemize}
    \item \emph{Analisi dei Requisiti:} completamento degli \emph{use case} per gli agenti README, changelog e OWASP, stesura della sezione di Tracciamento e completamento della tabella dei Requisiti;
    \item \emph{Piano di Qualifica:} stesura dei capitoli 2.1, 2.2 e 2.3;
    \item \emph{Norme di Progetto:} stesura del capitolo relativo allo sviluppo;
    \item \emph{Gestione documentale e repository:} aggiornamento del Glossario con introduzione di nuovi termini e creazione dei collegamenti tra documenti e glossario, sistemazione della struttura delle cartelle GitHub per i file \texttt{.tex} e sistemazione dei template dei documenti;
\end{itemize}

\paragraph{Preventivo orario ed economico}

I ruoli assegnati sono: Responsabile, Amministratore, Analista, Progettista, Programmatore e Verificatore.
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.9\textwidth}{|X|c|c|c|c|c|c|c|}
        \hline
        \textbf{Sprint 4} & \textbf{RE} & \textbf{AM} & \textbf{AN} &
        \textbf{PG} & \textbf{PR} & \textbf{VE} & \textbf{Totale} \\ \hline
        Andrea Fistarol & & & 4 & & & & \textbf{3} \\ \hline
        Dario Pirrone & & 5 & & & & & \textbf{0} \\ \hline
        Edoardo Granziero & 4 & & & & & & \textbf{0} \\ \hline
        Marco Barbiero & & & 5 & & & & \textbf{0} \\ \hline
        Samuele Bertin & & & & & & 3 & \textbf{0} \\ \hline
        Sara Ristovic & & & 5 & & & & \textbf{0} \\ \hline
        \textbf{Ore totali per ruolo} & \textbf{0} & \textbf{0} &
        \textbf{0} & \textbf{0} & \textbf{0} & \textbf{0} & \textbf{0}\\ \hline
    \end{tabularx}
    \caption{Preventivo orario - Sprint 4}
\end{table}
...
\begin{table}[H]
    ...
    \end{tabularx}
    \caption{Preventivo economico - Sprint 4}
\end{table}
